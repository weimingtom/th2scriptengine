...
void MAIN_SystemControl( void )
{
	...
	case MAIN_GAME:
		...
		if(KeyCond.trg.p) 
			Debug.pos   = !Debug.pos;		
		if(KeyCond.trg.n) 
			Debug.number = (Debug.number+1)%3;
		if(KeyCond.trg.s) 
			Debug.string = !Debug.string;
		if(KeyCond.trg.a) 
			Debug.scenario = (Debug.scenario+1)%3;
		if(KeyCond.trg.g) 
			Debug.game = !Debug.game;
		if(KeyCond.trg.r) 
			Debug.reg = (Debug.reg+1)%3;
		if(KeyCond.trg.c) 
	...
	case MAIN_GAME:
		...
		if(Debug.flag)
		{
			...
			if(Debug.string)
			{
				...
				{
					if(Debug.str[i])
						wsprintf( work, "s%d:%s",  i, Debug.str[i] );
					else			
						wsprintf( work, "s%d:NULL",i );
					DRW_DrawDmoji( dest, 8,  y, work,  0, MainWindow.draw_mode2 );	y+=8;
				}
			}
		}
...
{	
	if( (MainWindow.draw_mode == 8) && (MainWindow.draw_mode != MainWindow.draw_mode2) ){
		switch( MainWindow.draw_mode2 )
		{
			...
			case 16:	
				if( MAIN_DrawGraph(&MainWindow.vram_high) )	
					DRW_ChangeBMP_H2B( &MainWindow.vram_256, &MainWindow.vram_high );	
				break;
			
			case 24:	
				if( MAIN_DrawGraph(&MainWindow.vram_full) )	
					DRW_ChangeBMP_F2B( &MainWindow.vram_256, &MainWindow.vram_full );	
				break;
			
			case 32:	
				if( MAIN_DrawGraph(&MainWindow.vram_true) )	
					DRW_ChangeBMP_T2B( &MainWindow.vram_256, &MainWindow.vram_true );	
				break;
		}
	}
	else
	{
		...
		case 8:		
			MAIN_DrawGraph( &MainWindow.vram_256  );	
			break;

		case 16:	
			MAIN_DrawGraph( &MainWindow.vram_high );	
			break;

		case 24:	
			MAIN_DrawGraph( &MainWindow.vram_full );
			break;

		case 32:	
			MAIN_DrawGraph( &MainWindow.vram_true );
			break;
		}
	}
...
void MAIN_Loop( void )
{
	...
	if( FrameFormat == FRAME_COUNT )
	{
		...
		if ( next_time <= NowTime )
		{
			...
			if(MainWindow.save_disp) 
				MainWindow.draw_flag = 1;
			if(MainWindow.save_disp==2)	
				MainWindow.save_disp = 0;
			if( MainWindow.save_disp )	
				MainWindow.save_disp = 2;
			if( MainWindow.draw_flag == 2 )
				MainWindow.draw_flag = 0;
			...
			{
				if(skip_frame)	
					skip_cnt = (int)min( skip*frame/1000, 40 );
				else			
					skip_cnt = (int)min( skip*frame/1000, 10 );
