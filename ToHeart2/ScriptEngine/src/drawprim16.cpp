						...
						int r, int g, int b, int blnd )
{
	...
	if( blnd==0  ) 
		return TRUE;
	if( blnd==256) 
		return PRM_DrawRect_H_Std( dest, dx, dy, w, h, r,g, b );
	for( y=0; y < h ; y++, dest_p+=drev )
	{
					...
					DWORD wparam )
{
	...
	default:
	case DRW_NML:	
		PRM_DrawRect_H_Std( dest, dx, dy, w, h, r, g, b );			
		break;	

	case DRW_BLD2:	
		PRM_DrawRect_H_Bld( dest, dx, dy, w, h, r, g, b, blnd );	
		break;

	case DRW_ADD:	
		PRM_DrawRect_H_Add( dest, dx, dy, w, h, r, g, b );			
		break;	

	case DRW_SUB:	
		PRM_DrawRect_H_Sub( dest, dx, dy, w, h, r, g, b );			
		break;

	case DRW_MUL:	
		PRM_DrawRect_H_Mul( dest, dx, dy, w, h, r, g, b );			
		break;	

	case DRW_AMI2:	
		PRM_DrawRect_H_Ami( dest, dx, dy, w, h, r, g, b, blnd );	
		break;	
...
BOOL PRM_DrawFxLine_H_Mul( WORD *dest_p, MIN_MAX *mi, MIN_MAX *ma, RECT *clip, int r, int g, int b )
{
	...
	for( x=0 ; x < w ; x++, dest_p++ )
	{
		if( r<=BRT_NML)	
			...
			rr = ((( (mr-(*dest_p&mr))*r2 )>>7)+(*dest_p&mr))&mr;
		if( g<=BRT_NML)	
			gg = ( (     (*dest_p&mg) *g  )>>7              )&mg;
		else			
			gg = ((( (mg-(*dest_p&mg))*g2 )>>7)+(*dest_p&mg))&mg;
		if( b<=BRT_NML)	
			bb = ( (     (*dest_p&mb) *b  )>>7              )&mb;
		else			
			bb = ((( (mb-(*dest_p&mb))*b2 )>>7)+(*dest_p&mb))&mb;
		*dest_p = (rr|gg|bb);
	}
	return TRUE;
}
									...
									DWORD wparam )
{
	...
	default:
	case DRW_NML:	
		ret = PRM_DrawFLine_H_Std( dest, x1, y1, x2, y2, r, g, b );			
		break;	

	case DRW_BLD2:	
		ret = PRM_DrawFLine_H_Bld( dest, x1, y1, x2, y2, r, g, b, blnd );	
		break;	

	case DRW_ADD:	
		ret = PRM_DrawFLine_H_Add( dest, x1, y1, x2, y2, r, g, b );			
		break;	

	case DRW_SUB:	
		ret = PRM_DrawFLine_H_Sub( dest, x1, y1, x2, y2, r, g, b );			
		break;	

	case DRW_MUL:	
		ret = PRM_DrawFLine_H_Mul( dest, x1, y1, x2, y2, r, g, b );			
		break;	
									...
									RECT *clip, DWORD wparam )
{
	...
	default:
	case DRW_NML:	
		ret = PRM_DrawGLine_H_Std( dest, x1, y1, r1, g1, b1, x2, y2, r2, g2, b2 );			
		break;	

	case DRW_BLD2:	
		ret = PRM_DrawGLine_H_Bld( dest, x1, y1, r1, g1, b1, x2, y2, r2, g2, b2, blnd );	
		break;	

	case DRW_ADD:	
		ret = PRM_DrawGLine_H_Add( dest, x1, y1, r1, g1, b1, x2, y2, r2, g2, b2 );			
		break;	

	case DRW_SUB:	
		ret = PRM_DrawGLine_H_Sub( dest, x1, y1, r1, g1, b1, x2, y2, r2, g2, b2 );			
		break;	

	case DRW_MUL:	
		ret = PRM_DrawGLine_H_Mul( dest, x1, y1, r1, g1, b1, x2, y2, r2, g2, b2 );			
		break;	
