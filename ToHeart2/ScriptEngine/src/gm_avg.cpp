...
#include "keyboard.h"
#include "mouse.h"
#include "soundDS.h" //#include <soundDS.h>
#include "escript.h"
#include "font.h"
#include "movPlayer.h"
#include "main.h"
...
void AVG_LoadGameFlag( void )
{
	...
	{
		int	i;
		if(ESC_GameFlagBuf[20]!=1)	
			ESC_GameFlagBuf[20]=0;
		if(ESC_GameFlagBuf[21]!=1)	
			ESC_GameFlagBuf[21]=0;
		if(ESC_GameFlagBuf[22]!=1)	
			ESC_GameFlagBuf[22]=0;
		if(ESC_GameFlagBuf[80]>=10)	
			ESC_GameFlagBuf[80]=0;
		for(i=81;i<=87;i++)
		{
			if(ESC_GameFlagBuf[i]!=1)	
				ESC_GameFlagBuf[i]=0;
		}
		if(ESC_GameFlagBuf[95]!=1)	
			ESC_GameFlagBuf[95]=0;
		if(ESC_GameFlagBuf[98]!=1)	
			ESC_GameFlagBuf[98]=0;
		if(ESC_GameFlagBuf[99]!=1)	
			ESC_GameFlagBuf[99]=0;
...
BOOL AVG_WaitFrame( void )
{
	...
	if( WaitStruct.flag )
	{
		...
		case 0:	
			wait_max = AVG_EffCnt(WaitStruct.max);	
			break;
		
		case 1:	
			wait_max = AVG_EffCnt2(WaitStruct.max);	
			break;
		
		case 2:	
			wait_max = AVG_EffCnt3(WaitStruct.max);	
			break;
		}
		WaitStruct.count++;
...
void AVG_ControlWeather( void )
{
	...
	if( Weather.flag )
	{
		if(Weather.wind  < Weather.twind  )	
			Weather.wind  = Weather.wind + 0.1f;
		if(Weather.speed < Weather.tspeed )	
			Weather.speed = min( Weather.tspeed,  Weather.speed +1 );
		if(Weather.wind  > Weather.twind  )	
			Weather.wind  = Weather.wind - 0.1f;
		if(Weather.speed > Weather.tspeed )	
			Weather.speed = max( Weather.tspeed,  Weather.speed -1 );
		Weather.cnt++;
		if(Weather.reset)
			Weather.reset_cnt++;
...
void AVG_ControlLensFrea( void )
{
	...
	if( LensFrea.flag )
	{
		...
		{
			if(LensFrea.rev)
				dc = 96-96*LensFrea.count/lf_max-16+256;
			else			
				dc = 96*LensFrea.count/lf_max-16+256;
			if(LensFrea.light)
			{
				x2=DISP_X;
				x = SIN( dc+64 );
...
void AVG_SizukuPlayVoice( int vc_no, int vno, int char_no, int ex_no, int vol, int loop, int goin )
{
	...
	if(ex_no==-1)	
	{	
		wsprintf( fname, "%04d_%03d", vno, cno );			
	}
	else			
	{	
		wsprintf( fname, "%04d_%03d_%d", vno, cno, ex_no );	
	}
	if( -1>=cno || cno>=100)
	{
	...
	if(!goin)
	{
		...
		if(cno<=5)
		{
			if(CharVoiceFlag[cno]==OFF) 
				skip_flag=1;
		}
		else
		{
			if(cno==6 || cno==7 || cno==98) 
			{ 
				if(CharVoiceFlag[7]==OFF) 
					skip_flag=1; 
			}
			else if(cno==99)				
			{ 
				if(CharVoiceFlag[6]==OFF) 
					skip_flag=1; 
			}
		}
		if(skip_flag)
...
BOOL AVG_PlaySound( int vc_no, char *fname, int loop )
{
	...
	{
		if(loop)
			PlaySound( fname2, NULL, SND_FILENAME|SND_ASYNC|SND_LOOP );
		else	
			PlaySound( fname2, NULL, SND_FILENAME|SND_ASYNC );
	}
	else
	{
...
void AVG_PlayVoice( int vc_no, int cno, int sno, int vno, int vol, int loop, int a_cut, int test )
{
	...
	if(VI_EventVoiceNoAll < 0)
	{
		...
		{
			if(sno==-1)	
				sno = ScenarioNo/100*100 + VI_EventVoiceNo;
		}
	}
	else
	{
	...
	else
	{
		...
		{
			if(cno==28)	
				skip_voice = CharVoiceFlag[10]==OFF;
			else		
				skip_voice = CharVoiceFlag[cno-1]==OFF;
		}
	}
	if(test==0)
...
void AVG_ChangeDeviceBGM( int device )
{
	...
	{	
		AVG_PlayCD( PlayMusicNo, TRUE, TRUE );				
	}
}
...
void AVG_PlaySE2( int sno, int se_no, int fade, int loop, int vol )
{
	...
	{
		if(fade) 
			SeStruct[sno].dno = AVG_PlaySE( se_no, loop, 0 );
		else	 
			SeStruct[sno].dno = AVG_PlaySE( se_no, loop, vol );
...
void AVG_CloseStartConfigWindow( int cmax )
{
	...
	{
		if( Config.mode==CNF_OPEN )	
		{ 
			Config.cnt = cmax-Config.cnt;	
		}
		else						
		{ 
			Config.cnt = 0;				
...
int AVG_ControlConfigWindow( void )
{
	...
	if( Config.flag )
	{
		...
		case CNF_NORMAL:
			if(GameKey.u)	
				MUS_SetMousePosRect( MainWindow.hwnd, 1, (select<=0)? 0 : select-1 );
			if(GameKey.d)	
				MUS_SetMousePosRect( MainWindow.hwnd, 1, (select<0)? 0 : select+1 );
			for(i=0;i<4;i++)
	...
	if(FadeStruct.flag)
	{
		...
		if(FadeStruct.cnt>=max)
		{
			...
			if( !FadeStruct.disp )
			{
				...
				for(i=0; i<TXT_DISP ;i++)
				{
					if( FadeStruct.txt_flag[i] ) 
					{	
						DSP_SetTextDisp( i, ON );	
					}
				}
...
void AVG_SetLoadData( AVG_SAVE_DATA sdata )
{
	...
	if(BackStruct.zoom)	
		AVG_SetBackPosZoom( sdata.bs_x, sdata.bs_y, sdata.bs_w, sdata.bs_h );
	else				
		AVG_SetBackPos( sdata.bs_x, sdata.bs_y );
	...
	if( sdata.sw_flag )
	{
		...
		{
			if(sdata.sw_ex)	
				AVG_SetSelectMessageEx( sdata.sw_mes[i], sdata.sw_sno[i], sdata.sw_no[i], sdata.sw_value[i] );
			else			
				AVG_SetSelectMessage( sdata.sw_mes[i], sdata.sw_no[i], sdata.sw_value[i],0 );
		}
		AVG_SetHalfTone();
	}
	if(sdata.ws_flag)
...
void AVG_SetMovie( int mode, int no )
{
	...
	switch(mode)
	{
		case 0:
			...
			default:
			case 0:	
				movPlayerFrm->OpenMovie( MainWindow.hwnd, "TH2_OP_480x272_2M.avi", 0,76,800, 448 );	
				break;

			case 1:	
				movPlayerFrm->OpenMovie( MainWindow.hwnd, "TH2_OP_640x352_4M.avi", 0,76,800, 448 );	
				break;

			case 2:	
				movPlayerFrm->OpenMovie( MainWindow.hwnd, "TH2_OP_800x448_5M.avi", 0,76,800, 448 );	
				break;
			}
		...
		case 1:
			...
			default:
			case 0:	
				wsprintf( fname, "TH2_ED_%02d_640_1_5M.avi", no );	
				break;
			...
			case 2:	
				wsprintf( fname, "TH2_ED_%02d_800_3M.avi", no );	
				break;
			}
		...
		case 2:
			...
			default:
			case 0:	
				movPlayerFrm->OpenMovie( MainWindow.hwnd, "TH2_TR_480x360_2M.avi", 0,0,800, 600 );	
				break;

			case 1:	
				movPlayerFrm->OpenMovie( MainWindow.hwnd, "TH2_TR_640x480_4M.avi", 0,0,800, 600 );	
				break;
			
			case 2:	
				movPlayerFrm->OpenMovie( MainWindow.hwnd, "TH2_TR_800x600_5M.avi", 0,0,800, 600 );	
				break;
			}
		...
		case 3:
			...
			default:
			case 0:	
				movPlayerFrm->OpenMovie( MainWindow.hwnd, "Leaf_480x360_2M.avi", 0,0,800,600 );	
				break;
			
			case 1:
				movPlayerFrm->OpenMovie( MainWindow.hwnd, "Leaf_640x480_4M.avi", 0,0,800,600 );	
				break;
			
			case 2:	
				movPlayerFrm->OpenMovie( MainWindow.hwnd, "Leaf_800x600_5M.avi", 0,0,800,600 );	
				break;
			}
			break;
	}
...
void AVG_SetMapEvent( int chr_no, int pos, int type, TCHAR *filename )
{
		...
		case 1:
			if( AVG_ViewClock( 19 ) )
			{
				...
				for(i=0;i<5;i++)
				{
					...
					{
						if(i==1 || i==4) 
							wsprintf( fname, "map1%d%d.tga", i, sakura_type );
						else			 
							wsprintf( fname, "map1%d%d.tga", i, 0 );
			...
			if( EventFieldRoolCount==0 )
			{
				...
				if(GameKey.pup)
				{
					...
					while(1)
					{
						...
						if(EventFieldList[EventFieldCount])
							break;
					}
				}
				if(GameKey.pdown)
...
int AVG_ToHertDaySinkou( int script )
{
	...
	if( script==SCCODE_NOOPR && MAIN_GetScriptFlag()!=0)
	{
		...
		if( _month==3 && _days==20 ) 
			_syukujitu=1;
		if( _month==4 && _days==29 ) 
			_syukujitu=1;
		if( _month==5 && _days==3 ) 
			_syukujitu=1;
		if( _month==5 && _days==4 ) 
		...
		{
			case 0:	
				wsprintf( buf, "EV_%02d%02dMORNING.sdt",_month, _days );	
				break;	
			
			case 1:	
				wsprintf( buf, "EV_%02d%02dINTERVAL.sdt",_month, _days );	
				break;	
			
			case 2:	
				wsprintf( buf, "EV_%02d%02dLUNCH_BREAK.sdt",_month, _days );	
				break;	
			
			case 3:	
				wsprintf( buf, "EV_%02d%02dSCHOOL_HOURS.sdt",_month, _days );	
				break;	
			
			case 4:	
				wsprintf( buf, "EV_%02d%02dAFTER_SCHOOL.sdt",_month, _days );	
				break;
			...
			case 6:	wsprintf( buf, "EV_%02d%02dNIGHT.sdt",_month, _days );	
				break;	
		}
