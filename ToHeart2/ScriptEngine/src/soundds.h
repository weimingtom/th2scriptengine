...
struct ClSoundBuffer
{
	...
	~ClSoundBuffer()
	{
		...
		if(streamNum>=0)
			readFile->StreamCloseFile(arcFileNum,streamNum);
		if(hFadeThread) 
...
public:
	...
	void SetVolume(int handle,int value, int setting_20 );
...
inline void PlayEffect(int x,BYTE vol,BOOL bLoop=FALSE)
{
	int	loopCnt = 1;
	if(bLoop)loopCnt = 0;
	lpSoundDS->SetVolume(x,vol,TRUE);
