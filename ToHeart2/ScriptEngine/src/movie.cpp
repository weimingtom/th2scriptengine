...
void MOV_DebugBox( HWND hwnd, TCHAR *mes, ... )
{
	TCHAR	buf[512];
	if(hwnd==NULL) 
		hwnd = AviWindowHWND;
	wvsprintf( buf, mes, (char *)(&mes+1) );
	MessageBox( hwnd, buf, _T("警告"), MB_OK|MB_SETFOREGROUND );
}
...
int MOV_OpenFile( TCHAR *fname, int w, int h, int bit )
{
	int					i,size;
	int					ret=-1;
	...
	if( ret==-1 )
	{
		MOV_DebugBox( NULL, _T("AVIストリームが多過ぎます[4つまで]") );
		return -1;
	}
	AVIFileInit();
	{
	...
	case AVIERR_BADFORMAT:		
		MOV_DebugBox( NULL, _T("ファイルが破損しているか、Codecがインストールされていないため、読み取りできませんでした。") );	
		return -1;
	
	case AVIERR_MEMORY:			
		MOV_DebugBox( NULL, _T("メモリ不足のため、ファイルを開けませんでした。") );			
		return -1;

	case AVIERR_FILEREAD:		
		MOV_DebugBox( NULL, _T("ファイルの読み取り中にディスク エラーが発生しました。") );	
		return -1;

	case AVIERR_FILEOPEN:		
		MOV_DebugBox( NULL, _T("ファイルのオープン中にディスク エラーが発生しました。") );	
		return -1;
	
	case REGDB_E_CLASSNOTREG:	
		MOV_DebugBox( NULL, _T("AVIFileOpen 関数で指定されたファイルタイプを持つハンドラが、レジストリに見つかりませんでした ") );	
		return -1;
	
	case E_NOINTERFACE:
	default:					
		MOV_DebugBox( NULL, _T("何らかの原因により AVIFileOpen関数が失敗しました[%08x] "), hResult );		
		return -1;
	}
	...
	if(hResult)
	{
		MOV_DebugBox( NULL, _T("AVI情報の取得に失敗しました()") );
		return  -1;
	}
	for( i=0; i<(int)FileInfo.dwStreams ; i++ )
	{
		...
		{	
			as->bUseAudio = FALSE;
			OutputDebugString(_T("AVI音声は未サポートです\n"));
			break;
			as->bUseAudio = TRUE;
	...
	case 1:
		OutputDebugString( _T("指定フォーマットで描画できません。標準フォーマットで再生します。\n") );
		as->bUseVideo = TRUE;
		break;
	}	
	{
...
int MOV_SetVideoBufFormat( int mov_no, int w, int h, int bit, BITMAPINFOHEADER *bi )
{
	...
	if(w)	
		...
		bi->biWidth    = as->bi.biWidth;
	if(h)	
		bi->biHeight   = -h;
	else	
		bi->biHeight   = as->bi.biHeight;
	bi->biPlanes = 1;
	if(bit)	
		bi->biBitCount = bit;
	else	
		bi->biBitCount = as->bi.biBitCount;
...
DWORD MOV_GetVideoBuf( int mov_no, void **buf, int *w, int *h, int *bit )
{
	if( AviStruct[mov_no].flag && AviStruct[mov_no].bUseVideo )
	{
		*buf = AviStruct[mov_no].VideoBuf;
		if(bit)
			*bit = AviStruct[mov_no].bi.biBitCount;
		if(w)  
			*w   = AviStruct[mov_no].bi.biWidth;
		if(h)  
			*h   = abs(AviStruct[mov_no].bi.biHeight);
