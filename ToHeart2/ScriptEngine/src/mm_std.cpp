...
void memcpy2( void *dst, const void *src, int size )
{
	...
	else if(SSE_FP_Flag )
	{
		...
		if(size-size2*16) 
			CopyMemory( (char*)(dst)+size2*16, (char*)(src)+size2*16, size-size2*16 );
	}
	else if(MMX_Flag)
	{
		if(size2)
...
void DebugBox(HWND hwnd, TCHAR *mes, ...)
{
	char	buf[512];
	wvsprintf( buf, mes, (char *)(&mes+1) );
#ifdef _DEBUG
	MessageBox2( hwnd, MB_OK|MB_SETFOREGROUND, _T("警告"), buf );
#else
	MessageBox2( hwnd, MB_OK|MB_SETFOREGROUND, _T("警告"), buf );
#endif
}

int MessageBox2( HWND hwnd, DWORD flag, TCHAR *title, TCHAR *mes, ...)
{
	char	buf[512];
...
int CheckBox( HWND hwnd, TCHAR *title, TCHAR *mes, ... )
{
	char	buf[512];
	wvsprintf( buf, mes, (char *)(&mes+1) );
...
{
	HWND	hwnd;
	if( (hwnd=FindWindow(NULL, _T("Play Control")))==NULL )
	{
		if( (hwnd=FindWindow(NULL, _T("ボリューム コントロール")))==NULL )
		{
			if( (hwnd=FindWindow(NULL, _T("ボリューム コントロール")))==NULL )
			{
				if( (hwnd=FindWindow(NULL, _T("デスクトップ スピーカー")))==NULL )
				{
					if( (hwnd=FindWindow(NULL,_T("デスクトップ スピーカー")))==NULL )
					{
						if( (hwnd=FindWindow(NULL, _T("Master Volume")))==NULL )
						{
							hwnd=FindWindow(NULL, _T("Volume Control"));
						}
					}
				}
			}
...
void STD_GetWindowPos( HWND hd, int *sx, int *sy ) //TODO:long *
{
	RECT rect = {0};
...
DWORD STD_GetFileSize( TCHAR *fname )
{
	HANDLE		fh;
	DWORD		dummy, size;
...
BOOL STD_WriteFilePos( TCHAR *fname, char *src, DWORD pos, DWORD size )
{
...
BOOL STD_WriteFile( TCHAR *fname, char *src, DWORD size )
{
	HANDLE		fh;
	BOOL		dummy;
...
BOOL STD_ApendFile( TCHAR *fname, char *src, DWORD size )
{
	HANDLE		fh;
	BOOL		dummy;
	fh = CREATE_APEND_FILE( fname );
...
BOOL STD_ReadFilePos( TCHAR *fname, char *buf, DWORD pos, DWORD size )
{
	HANDLE		fh;
	DWORD		dummy;
	fh = CREATE_READ_FILE( fname );
...
BOOL STD_ReadFile( TCHAR *fname, char *buf, DWORD size )
{
	HANDLE		fh;
	DWORD		dummy;
	fh = CREATE_READ_FILE( fname );
...
DWORD STD_ReadFileMax( TCHAR *fname, char **buf )
{
	HANDLE		fh;
	DWORD		dummy, size;
	fh = CREATE_READ_FILE( fname );
	...
	if( dummy != size )
	{
		DebugPrintf( _T("ファイルサイズと読みこみサイズが違うなり、何ゆえ？") );
	}
	return size;
}

BOOL STD_DeleteFile( TCHAR *fname )
{
	HANDLE		fh;
...
BOOL STD_CheckFile( TCHAR *fname )
{
	HANDLE		fh;
	fh = CREATE_READ_FILE( fname );
	if( fh == INVALID_HANDLE_VALUE ) 
...
BOOL STD_DeleteDir( TCHAR *src )
{
	...
	while( FindNextFile( hFind, &FindFileData ) )
	{
		...
		{
			wsprintf( src_buf, _T("%s\\%s"), src, FindFileData.cFileName );
			if( !STD_DeleteDir( src_buf ) )
			{
				DebugBox(NULL, _T("フォルダ[%s]が削除できません。"),src_buf);
			}
		}
		else
		{
			...
			{
				DebugBox(NULL, _T("ファイル[%s]が削除できません。"),src_buf);
			}
		}
	}
	FindClose( hFind );
...
BOOL STD_CopyDir( TCHAR *src, TCHAR *dst, BOOL uwagaki )
{
	TCHAR			src_buf[1024];
	TCHAR			dst_buf[1024];
	WIN32_FIND_DATA	FindFileData;
	...
	while( FindNextFile( hFind, &FindFileData ) )
	{
		...
		{
			wsprintf( src_buf, _T("%s\\%s"), src, FindFileData.cFileName );
			wsprintf( dst_buf, _T("%s\\%s"), dst, FindFileData.cFileName );	
...
BOOL STD_CopyDir_First( TCHAR *src, TCHAR *dst, BOOL uwagaki, COPY_DIR_DATA *cdd )
{
	strcpy( cdd->src, src );
	strcpy( cdd->dst, dst );
	cdd->uwagaki = uwagaki;
...
static BOOL STD_CopyDir_Next2( TCHAR *src, TCHAR *dst, COPY_DIR_DATA *cdd )
{
	TCHAR			src_buf[1024];
	TCHAR			dst_buf[1024];
	WIN32_FIND_DATA	FindFileData;
...
static BOOL SaerchNextFile2( TCHAR *src_dir, WIN32_FIND_DATA_LIST **list, SEARCH_DIR_DATA *sdd )
{
	TCHAR src_buf[1024];
	if(*list==NULL)
	{
	...
	while( *list )
	{
		...
		case SEARCH_DIR_INIT:
			wsprintf( src_buf, _T("%s%s"), src_dir, "*" );
		...
		case SEARCH_DIR:
			...
			if( (*list)->fnf_ret )
			{
				...
				{
					wsprintf( src_buf, _T("%s%s\\"), src_dir, (*list)->data.cFileName );
		...
		case SEARCH_FILE_INIT://直下ファイルを検索
			wsprintf( src_buf, _T("%s%s"), src_dir, sdd->file );//全てのファイルを検出
			(*list)->handle = FindFirstFile( src_buf, &(*list)->data );//.
			if( (*list)->handle == INVALID_HANDLE_VALUE )
			{
				GFree( *list );
...
BOOL InitSaerchFile( char *src, BOOL subdir_chk, SEARCH_DIR_DATA *sdd )
{
	...
	wsprintf( sdd->dir,  _T("%s%s"), drive, dir );
	wsprintf( sdd->file, _T("%s%s"), fname, ext );
...
static TCHAR DefaultDir[MAX_PATH];
static TCHAR DefaultExt[8];
// ---------------------------------------------------------------------------
//	デフォルトデレクトリ設定
// ---------------------------------------------------------------------------
void STD_SetDefaultDir( TCHAR *str, int flag )
{
	int	i;
...
//	デフォルト拡張子設定
// ---------------------------------------------------------------------------
void STD_SetDefaultExt( TCHAR *str)
{
...
// ---------------------------------------------------------------------------
void STD_LoadDefaultDir( TCHAR *regkey )
{
	SetRegistryKeyEx( regkey );
	ReadRegistryStr( _T("defdir"), DefaultDir );
	ReadRegistryStr( _T("defext"), DefaultExt );
}

// ---------------------------------------------------------------------------
//	デフォルトディレクトリ／拡張子、レジストリセーブ
// ---------------------------------------------------------------------------
void STD_SaveDefaultDir( TCHAR *regkey )
{
	SetRegistryKeyEx( regkey );
	WriteRegistryStr( _T("defdir"), DefaultDir );
	WriteRegistryStr( _T("defext"), DefaultExt );
}

// ---------------------------------------------------------------------------
//	オープンファイルダイアログ
// ---------------------------------------------------------------------------
TCHAR *STD_GetOpenFileName( HWND hwnd, TCHAR *ff)
{
	OPENFILENAME	fname;
	static TCHAR	fn[MAX_PATH];
	TCHAR			filename[64];
	fn[0] = '\0';
...
//	セーブファイルダイアログ
// ---------------------------------------------------------------------------
TCHAR *STD_GetSaveFileName( HWND hwnd, TCHAR *ff)
{
	OPENFILENAME	fname;
	static TCHAR	fn[MAX_PATH];
	TCHAR			filename[64];
...
BOOL STD_LuckupDir( HWND hwnd, TCHAR *dir )
{
...
void SetIniFileName( TCHAR *dir, TCHAR *ini_fname, BOOL *exist )
{
	if(dir)
	{
		wsprintf( IniFileName, _T("%s\\%s.ini"), dir, ini_fname );
	}
	else
	{
		wsprintf( IniFileName, _T(".\\%s.ini"), ini_fname );
	}
	if(exist)
	{
...
// ---------------------------------------------------------------------------
int GetIniFileKeyList( TCHAR *key_list, int size, TCHAR *sec_name, TCHAR *ini_fname )
{
	if( ini_fname==NULL)
	{
		if(IniFileName[0]==0)
		{
			ini_fname = _T("default.ini");
		}
		else
		{
	...
	GetPrivateProfileString( sec_name, NULL, _T("NO_KEY"), key_list, size, ini_fname );
	return !strcmp( key_list, "NO_KEY" ); //TODO:???
}

// ---------------------------------------------------------------------------
//	iniファイルからSECTIONのリストを受け取る
// ---------------------------------------------------------------------------
int GetIniFileSectionList( TCHAR *sec_list, int size, TCHAR *ini_fname )
{
	if( ini_fname==NULL)
	{
		if(IniFileName[0]==0)
	...
	GetPrivateProfileString( NULL, NULL, _T("NO_SEC"), sec_list, size, ini_fname );
	return !strcmp( sec_list, "NO_SEC" ); //TODO:???
}
...
// ---------------------------------------------------------------------------
int WriteIniFileAmount( TCHAR *vname, int amount, TCHAR *sec_name, TCHAR *ini_fname )
{
	int		ret;
	TCHAR	int_buf[16];
	if( ini_fname==NULL)
...
// ---------------------------------------------------------------------------
int ReadIniFileAmount( TCHAR *vname, int def, TCHAR *sec_name, TCHAR *ini_fname )
{
	if( ini_fname==NULL)
	{
		if(IniFileName[0]==0)
...
//	iniファイルにデータを書き込む
// ---------------------------------------------------------------------------
int WriteIniFileData( TCHAR *vname, char *data, int size, TCHAR *sec_name, TCHAR *ini_fname )
{
	int		ret;
	if( ini_fname==NULL)
	{
...
//	iniファイルからデータを読み取る
// ---------------------------------------------------------------------------
int ReadIniFileData( TCHAR *vname, char *data, int size, TCHAR *sec_name, TCHAR *ini_fname )
{
	if( ini_fname==NULL)
	{
		if(IniFileName[0]==0)
...
//	iniファイルに文字を書き込む
// ---------------------------------------------------------------------------
int WriteIniFileStr( TCHAR *vname, char *str, TCHAR *sec_name, TCHAR *ini_fname )
{
	int		ret;
	TCHAR	buf[1024];
...
//	iniファイルから文字を読み取る
// ---------------------------------------------------------------------------
int ReadIniFileStr( TCHAR *vname, char *str, TCHAR *def, int size, TCHAR *sec_name, TCHAR *ini_fname )
{
	char	*buf = (char*)GAlloc( size ); //TODO:???
	if( ini_fname==NULL)
	{
		if(IniFileName[0]==0)
		{
...
//	iniファイルから指定KEYを削除する
// ---------------------------------------------------------------------------
int DeleteIniFileKey( TCHAR *vname, TCHAR *sec_name, TCHAR *ini_fname )
{
	int		ret;
	if( ini_fname==NULL)
	{
...
// ---------------------------------------------------------------------------
int DeleteIniFileSection( TCHAR *sec_name, TCHAR *ini_fname )
{
	int		ret;
	if( ini_fname==NULL)
	{
...
//++++++ レジストリ制御 +++++++++++++++++
static TCHAR RegistryKey[MAX_PATH];

// ---------------------------------------------------------------------------
//	レジストリキーを設定する
// ---------------------------------------------------------------------------
void SetRegistryKey( TCHAR *key)
{
	wsprintf( RegistryKey, _T("%s"), key);
}

void SetRegistryKeyEx( TCHAR *key)
{
	wsprintf( RegistryKey, _T("SoftWare\\%s"), key);
}

// ---------------------------------------------------------------------------
//	レジストリに値を書き込む
// ---------------------------------------------------------------------------
int WriteRegistryAmount( TCHAR *vname, long amount )
{
	int   ret;
	HKEY  key;
	DWORD size;
...
//	レジストリから値を読み取る
// ---------------------------------------------------------------------------
int ReadRegistryAmount( TCHAR *vname, long *amount )
{
...
//	レジストリに文字を書き込む
// ---------------------------------------------------------------------------
int WriteRegistryData( TCHAR *vname, char *data, int size )
{
	int   ret;
	HKEY  key;
	DWORD dmy;
...
//	レジストリから文字を読み取る
// ---------------------------------------------------------------------------
int ReadRegistryData( TCHAR *vname, char *data, int size )
{
	int   ret;
	HKEY  key;
...
//	レジストリに文字を書き込む
// ---------------------------------------------------------------------------
int WriteRegistryStr( TCHAR *vname, TCHAR *str )
{
	int   ret;
	HKEY  key;
	DWORD dmy;
	...
	if( ret==ERROR_SUCCESS) 
	{
		ret = RegSetValueEx( key, vname, 0, REG_SZ, (unsigned char *)str, _tcslen(str)+1 ); //strlen(str)+1
		RegCloseKey(key);
	} 
	else
	{
...
//	レジストリから文字を読み取る
// ---------------------------------------------------------------------------
int ReadRegistryStr(TCHAR *vname, TCHAR *str )
{
	int		ret;
...
//	レジストリを削除する
// ---------------------------------------------------------------------------
int DeleteRegistry(TCHAR *vname)
{
	int   ret;
	HKEY  key;
	ret = RegOpenKeyEx( HKEY_CURRENT_USER, RegistryKey, 0, KEY_ALL_ACCESS, &key);
...
BOOL STD_FiilMemory( void *buf, int size, int width, int code )
{
	...
	{
	case 1:	
		FillMemory( buf, size, code );			
		break;

	case 2:	
		FiilMemory_Word(  (u_short *)buf, size, code );		
		break;

	case 3:	
		FiilMemory_3Byte( (char *)buf, size, code );	
		break;

	case 4:	
		FiilMemory_Dword( (u_long *)buf, size, code );	
		break;
...
//	数字を文字列に変換(１０進１６桁)
//-------------------------------------------------------
TCHAR* Digit2Str(int digit, int zero, int lr)
{
	int			dg=1;
	int			p=0;
	int			digit2=digit;
	...
	{
		if( zero )	
			wsprintf( str, _T("%016d"), digit );
		else		
			wsprintf( str, _T("%16d"), digit );
	}
	else
	{
		wsprintf( str, _T("%d"), digit );
	}
	return str;
}

TCHAR *Digit2Str2(int digit, int zero, int lr)
{
	int			dg=1;
	int			p=0;
	int			digit2=digit;
	...
	{
		if( zero )	
			wsprintf( str, _T("%02d"), digit );
		else		
			wsprintf( str, _T("%2d"), digit );
	}
	else
	{
		wsprintf( str, _T("%d"), digit );
...
//	return ;
	...
	GetCpuClock();
#ifndef _UNICODE
	DebugPrintf(_T("VenderID  %s\n"), szVenderID);
	DebugPrintf(_T("CpuName   %s\n"), szCpuName);
#else
	wchar_t wVenderID[13] = {0};
	wchar_t wCpuName[49] = {0};
	MultiByteToWideChar(CP_ACP, 0, szVenderID, sizeof(szVenderID), wVenderID, 13);
	MultiByteToWideChar(CP_ACP, 0, szCpuName, sizeof(szCpuName), wCpuName, 49);
	DebugPrintf(_T("VenderID  %s\n"), wVenderID);
	DebugPrintf(_T("CpuName   %s\n"), wCpuName);
#endif
	DebugPrintf(_T("TypeID %d, FamilyID %d, ModelID %d, SteppingID %d\n"), TypeID, FamilyID, ModelID, SteppingID);
	DebugPrintf(_T("CpuClock %dMHz\n"), fCpuClock);
	DebugPrintf(_T("\n"));
	DebugPrintf(_T("CPUID       %d\n"), CPUID_Flag);		//CPUIDサポートの有無
	DebugPrintf(_T("CPUIDMax    %x\n"), CPUIDMax);		//CPUID標準機能のサポート最大値
	DebugPrintf(_T("CPUIDExtMax %x\n"), CPUIDExtMax);	//CPUID拡張機能のサポート最大値
	DebugPrintf(_T("FPU    %d\n"), FPU_Flag);				//浮動小数点演算器の有無
	DebugPrintf(_T("TSC    %d\n"), TSC_Flag);				//RDTSC命令の有無
	DebugPrintf(_T("MSR    %d\n"), MSR_Flag);				//RDMSR,WRMSR命令の有無
	DebugPrintf(_T("CMOV   %d\n"), CMOV_Flag);				//CMOV命令の有無
	DebugPrintf(_T("MMX    %d\n"), MMX_Flag);				//MMX命令の有無
	DebugPrintf(_T("3DNow! %d\n"), _3DNow_Flag);			//3DNow!命令の有無
	DebugPrintf(_T("SSE_INT    %d\n"), SSE_INT_Flag);		//SSE integer(=MMX拡張)命令の有無
	DebugPrintf(_T("SSE_FP     %d\n"), SSE_FP_Flag);		//SSE floating-point命令の有無
	DebugPrintf(_T("3DNow! Ext %d\n"), _3DNowExt_Flag);		//3DNow!拡張命令の有無
	DebugPrintf(_T("FCMOV      %d\n"), FPU_Flag & CMOV_Flag);//FCMOV命令の有無
/*
	DWORD			cpu_id = 0;
	DWORD			cpu_model = 0;
	DWORD			cpu_family = 0;
...
int STD_GetOSVer( void )
{
	...
	{
		if( OSver.dwMajorVersion<=2 )		
			return 256;//95以前
		else if(OSver.dwMajorVersion==3)	
			return 257;//win95
		else if(OSver.dwMajorVersion==4)	
			return 258;//win98,SE,ME
		else								
			return 260;//ME以降
	}
}

TCHAR *STD_GetOSName(void)
{
	...
	if( OSver.dwPlatformId==VER_PLATFORM_WIN32_NT )
	{
		if( OSver.dwMajorVersion<=4 )
		{//NT4以前
			wsprintf( buf, _T("Windows NT%d.%d (Build:%d) %s "), OSver.dwMajorVersion, OSver.dwMinorVersion, OSver.dwBuildNumber, OSver.szCSDVersion );
		}
		else if(OSver.dwMajorVersion==5)
		{//W2K,XP
			if(OSver.dwMinorVersion==0)
			{//W2K
				wsprintf( buf, _T("Windows 2000 %d.%d (Build:%d) %s "), OSver.dwMajorVersion, OSver.dwMinorVersion, OSver.dwBuildNumber, OSver.szCSDVersion );
			}
			else if(OSver.dwMinorVersion==1)
			{//XP
				wsprintf( buf, _T("Windows XP %d.%d (Build:%d) %s "), OSver.dwMajorVersion, OSver.dwMinorVersion, OSver.dwBuildNumber, OSver.szCSDVersion );
			}
			else
			{
				wsprintf( buf, _T("Windows XP以降 %d.%d (Build:%d) %s "), OSver.dwMajorVersion, OSver.dwMinorVersion, OSver.dwBuildNumber, OSver.szCSDVersion );
			}
		}
		else 
		{//XP以降
	...
	else
	{
		...
		{//win95
			wsprintf( buf, _T("Windows %d.%d (Build:%d) %s "), OSver.dwMajorVersion, OSver.dwMinorVersion, OSver.dwBuildNumber, OSver.szCSDVersion );
		}
		else if(OSver.dwMajorVersion==4)
		{//win98,SE,ME
			if(OSver.dwMinorVersion>=90)
			{//
				wsprintf( buf, _T("Windows ME %d.%d (Build:%d) %s "), OSver.dwMajorVersion, OSver.dwMinorVersion, OSver.dwBuildNumber, OSver.szCSDVersion );
			}
			else if(OSver.dwMinorVersion>=10)
			{
				wsprintf( buf, _T("Windows 98 %d.%d (Build:%d) %s "), OSver.dwMajorVersion, OSver.dwMinorVersion, OSver.dwBuildNumber, OSver.szCSDVersion );
			}
			else
			{
				wsprintf( buf, _T("Windows 95 %d.%d (Build:%d) %s "), OSver.dwMajorVersion, OSver.dwMinorVersion, OSver.dwBuildNumber, OSver.szCSDVersion );
			}
		}
		else
		{//ME以降
			wsprintf( buf, _T("Windows ME以降 %d.%d (Build:%d) %s "), OSver.dwMajorVersion, OSver.dwMinorVersion, OSver.dwBuildNumber, OSver.szCSDVersion );
		}
	}
	return buf;
}
