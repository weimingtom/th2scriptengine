...
BOOL DRW_DrawBMP_FTT( DRAW_OBJECT dobj, int interless )
{
	...
	if( dest->buf==NULL ) 
	{ 
		DebugPrintf(_T("描画領域がありません[DRW_DrawBMP_FTT]\n")); 
		return FALSE; 
	}
	if( src1->buf==NULL ) 
	{ 
		DebugPrintf(_T("画像領域1がありません[DRW_DrawBMP_FTT]\n")); 
		return FALSE; 
	}
	if( src2->buf==NULL ) 
	{ 
		DebugPrintf(_T("画像領域2がありません[DRW_DrawBMP_FTT]\n")); 
		return FALSE; 
	}
	...
	if( dobj.clip!=(void*)-1 )
	{
		if( !ClipRectS2(&dobj.dx,  &dobj.dy,  dest->sx, dest->sy,
						...
						&dobj.s2x, &dobj.s2y, src2->sx, src2->sy,  &dobj.s2w, &dobj.s2h, dobj.clip ) ) 
			return FALSE;
	}
	if( dobj2.clip!=(void*)-1 )
	{
		...
		if( !ClipRect(&dobj2.dx,  &dobj2.dy,  dest->sx, dest->sy,
						&dobj2.s1x, &dobj2.s1y, src1->sx, src1->sy,  &dobj2.dw, &dobj2.dh, dobj2.clip ) ) 
			dobj2.dest=NULL;
	}
	if( dobj3.clip!=(void*)-1 )
	{
		...
		if( !ClipRect(&dobj3.dx,  &dobj3.dy,  dest->sx, dest->sy,
						&dobj3.s1x, &dobj3.s1y, src1->sx, src1->sy,  &dobj3.dw, &dobj3.dh, dobj3.clip ) ) 
			dobj3.dest=NULL;
	}
	if( dobj4.clip!=(void*)-1 )
	{
		...
		if( !ClipRect(&dobj4.dx,  &dobj4.dy,  dest->sx, dest->sy,
						&dobj4.s1x, &dobj4.s1y, src1->sx, src1->sy,  &dobj4.dw, &dobj4.dh, dobj4.clip ) ) 
			dobj4.dest=NULL;
	}
	if( dobj5.clip!=(void*)-1 )
	{
		...
		if( !ClipRect(&dobj5.dx,  &dobj5.dy,  dest->sx, dest->sy,
						&dobj5.s1x, &dobj5.s1y, src1->sx, src1->sy,  &dobj5.dw, &dobj5.dh, dobj5.clip ) ) 
			dobj5.dest=NULL;
	}
	...
	case DRW_BLD2:
		if(dobj2.dest)
			DRW_DrawBMP_FT_Bld(&dobj2,interless);
		if(dobj3.dest) 
			DRW_DrawBMP_FT_Bld(&dobj3,interless);
		if(dobj4.dest) 
			DRW_DrawBMP_FT_Bld(&dobj4,interless);
		if(dobj5.dest) 
			DRW_DrawBMP_FT_Bld(&dobj5,interless);
		DRW_DrawBMP_FTT_Bld(&dobj,interless);
		break;	

	case DRW_AMI2:
		if(dobj2.dest) 
			DRW_DrawBMP_FT_Ami(&dobj2,interless);
		if(dobj3.dest) 
			DRW_DrawBMP_FT_Ami(&dobj3,interless);
		if(dobj4.dest) 
			DRW_DrawBMP_FT_Ami(&dobj4,interless);
		if(dobj5.dest) 
			DRW_DrawBMP_FT_Ami(&dobj5,interless);
	...
	case DRW_LPP2+3:	case DRW_LPP2+4:	case DRW_LPP2+5:	
		if(dobj2.dest) 
			DRW_DrawBMP_FT_Lpp(&dobj2,interless);
		if(dobj3.dest) 
			DRW_DrawBMP_FT_Lpp(&dobj3,interless);
		if(dobj4.dest) 
			DRW_DrawBMP_FT_Lpp(&dobj4,interless);
		if(dobj5.dest)
			DRW_DrawBMP_FT_Lpp(&dobj5,interless);
...
BOOL DRW_DrawBMP_FFB( DRAW_OBJECT dobj, int interless )
{
	...
	default:
	case DRW_BLD2:	
		DRW_DrawBMP_FFB_Ptn(&dobj,interless);	
		break;	
	}
	return ret;
}
...
BOOL DRW_DrawBMP_FFBM( DRAW_OBJECT dobj, int interless )
{
	...
	if( dest->buf==NULL ) 
	{ 
		DebugPrintf( _T("描画領域がありません[DRW_DrawBMP_FFBM]\n") ); 
		return FALSE; 
	}
	if( src1->buf==NULL ) 
	{ 
		DebugPrintf( _T("画像領域1がありません[DRW_DrawBMP_FFBM]\n") ); 
		return FALSE; 
	}
	if( src2->buf==NULL ) 
	{ 
		DebugPrintf( _T("画像領域2がありません[DRW_DrawBMP_FFBM]\n") ); 
		return FALSE; 
	}
	if( mask->buf==NULL ) 
	{ 
		DebugPrintf( _T("マスク領域がありません[DRW_DrawBMP_FFBM]\n") ); 
		return FALSE; 
	}
	if( dobj.clip!=(void*)-1 )
	{
	...
	default:
	case DRW_BLD2:	
		DRW_DrawBMP_FFBM_Ptn(&dobj,interless);	
		break;	
	}
	return ret;
}
...
BOOL DRW_DrawBMP_FT( DRAW_OBJECT dobj, int interless )
{
	...
	if( dest->buf==NULL ) 
	{	
		DebugPrintf(_T("描画領域がありません[DRW_DrawBMP_FT]\n")); 
		return FALSE;
	}
	if( src->buf==NULL  ) 
	{ 
		DebugPrintf(_T("画像領域がありません[DRW_DrawBMP_FT]\n")); 
		return FALSE; 
	}
	if( dobj.clip!=(void*)-1 )
	{
		if( !ClipRect( &dobj.dx,  &dobj.dy,  dest->sx, dest->sy,
					   &dobj.s1x, &dobj.s1y, src->sx,  src->sy,  &dobj.dw, &dobj.dh, dobj.clip ) ) 
			return FALSE;
	}
	...
	default:
	case DRW_NML:	
		DRW_DrawBMP_FT_Std( &dobj,interless );	
		break;	

	case DRW_BLD2:	
		DRW_DrawBMP_FT_Bld( &dobj,interless );	
		break;	

	case DRW_ADD:	
		DRW_DrawBMP_FT_Add( &dobj,interless );
		break;	

	case DRW_OOI:	
		DRW_DrawBMP_FT_Ooi( &dobj,interless );	
		break;	

	case DRW_SUB:	
		DRW_DrawBMP_FT_Sub( &dobj,interless );
		break;	

	case DRW_MUL:	
		DRW_DrawBMP_FT_Mul( &dobj,interless );	
		break;	

	case DRW_AMI2:	
		DRW_DrawBMP_FT_Ami( &dobj,interless );	
		break;	

	case DRW_NIS2:	
		DRW_DrawBMP_FT_Nis( &dobj,interless );
		break;	

	case DRW_NEG:	
		DRW_DrawBMP_FT_Neg(	&dobj,interless );	
		break;	

	case DRW_VIV2:	
		DRW_DrawBMP_FT_Viv( &dobj,interless );
		break;	

	case DRW_BOM2:	
		DRW_DrawBMP_FT_Bom(	&dobj,interless );
		break;	
...
{
	int i, j, k, x, y;
	if( RipleW==w && RipleH==h && RipleCX==cx && RipleCY==cy ) 
		return;
	RipleW=w;
	RipleH=h;
	RipleCX=cx;
		...
		{	
			for( y=0; y < h ; y++, dest_p+=drev, src_p+=srev )
			{
				if( INTERLESS_CHECK )
				{
					for( x=0 ; x < w ; x++, dest_p++, src_p+=xinc )
					{
						if(dest_p->b > btbl[src_p->b]) 
							dest_p->b = btbl[src_p->b];
						if(dest_p->g > gtbl[src_p->g]) 
							dest_p->g = gtbl[src_p->g];
						if(dest_p->r > rtbl[src_p->r]) 
							dest_p->r = rtbl[src_p->r];
					}
				}
				else
...
BOOL DRW_DrawBMP_FF_Wap( DRAW_OBJECT *dobj, int interless )
{
	...
	case 1:
		xxx[0*6+0] = 3*sw*r/rmax/3;	
		yyy[0*6+0] = 3*sh*r/rmax/3;
		xxx[0*6+1] = r*2;	
		yyy[0*6+1] = 2*sh*r/rmax/3;
		xxx[0*6+2] = r;		
		yyy[0*6+2] = 1*sh*r/rmax/3;
		xxx[0*6+3] = -r;	
		yyy[0*6+3] = 1*sh*r/rmax/3;
		xxx[0*6+4] = -r*2;	
		yyy[0*6+4] = 2*sh*r/rmax/3;
		xxx[0*6+5] = -3*sw*r/rmax/3;	
		yyy[0*6+5] = 3*sh*r/rmax/3;
		xxx[1*6+0] = 2*sw*r/rmax/3;	
		yyy[1*6+0] = r*2;
		xxx[1*6+1] = r;		
		yyy[1*6+1] = r;
		xxx[1*6+2] = r/2;	
		...
		xxx[1*6+4] = -r;	
		yyy[1*6+4] = r;
		xxx[1*6+5] = -2*sw*r/rmax/3;	
		yyy[1*6+5] = r*2;
		xxx[2*6+0] = 1*sw*r/rmax/3;		
		yyy[2*6+0] = r;
		xxx[2*6+1] = r/2;	
		yyy[2*6+1] = r/2;
		xxx[2*6+2] = -r;	
		...
		xxx[2*6+4] = -r/2;	
		yyy[2*6+4] = r/2;
		xxx[2*6+5] = -1*sw*r/rmax/3;	
		yyy[2*6+5] = r;
		xxx[3*6+0] = 1*sw*r/rmax/3;		
		yyy[3*6+0] = -r;
		xxx[3*6+1] = r/2;	
		yyy[3*6+1] = -r/2;
		xxx[3*6+2] = -r;	
		...
		xxx[3*6+4] = -r/2;	
		yyy[3*6+4] = -r/2;
		xxx[3*6+5] = -1*sw*r/rmax/3;	
		yyy[3*6+5] = -r;
		xxx[4*6+0] = 2*sw*r/rmax/3;	
		yyy[4*6+0] = -r*2;
		xxx[4*6+1] = r;		
		yyy[4*6+1] = -r;
		xxx[4*6+2] = r/2;	
		...
		xxx[4*6+4] = -r;	
		yyy[4*6+4] = -r;
		xxx[4*6+5] = -2*sw*r/rmax/3;	
		yyy[4*6+5] = -r*2;
		xxx[5*6+0] = 3*sw*r/rmax/3;	
		yyy[5*6+0] = -3*sh*r/rmax/3;
		xxx[5*6+1] = r*2;	
		yyy[5*6+1] = -2*sh*r/rmax/3;
		xxx[5*6+2] = r*1;	
		yyy[5*6+2] = -1*sh*r/rmax/3;
		xxx[5*6+3] = -r*1;	
		yyy[5*6+3] = -1*sh*r/rmax/3;
		xxx[5*6+4] = -r*2;	
		yyy[5*6+4] = -2*sh*r/rmax/3;
		xxx[5*6+5] = -3*sw*r/rmax/3;	
		yyy[5*6+5] = -3*sh*r/rmax/3;
		for(i=0;i<5;i++)
		{
			for(j=0;j<5;j++)
			{
...
BOOL DRW_DrawBMP_FF( DRAW_OBJECT dobj, int interless )
{
	...
	if( dest->buf==NULL ) 
	{	
		DebugPrintf(_T("描画領域がありません[DRW_DrawBMP_FF]\n")); 
		return FALSE; 
	}
	if( src->buf==NULL  ) 
	{ 
		DebugPrintf(_T("画像領域がありません[DRW_DrawBMP_FF]\n")); 
		return FALSE; 
	}
	if( dobj.clip!=(void*)-1 )
	{
	...
	default:
	case DRW_NML:	
		DRW_DrawBMP_FF_Std( &dobj, interless );	
		break;	

	case DRW_BLD2:	
		DRW_DrawBMP_FF_Bld( &dobj, interless );	
		break;	

	case DRW_ADD:	
		DRW_DrawBMP_FF_Add( &dobj, interless );	
		break;	

	case DRW_OOI:	
		DRW_DrawBMP_FF_Ooi( &dobj, interless );	
		break;	

	case DRW_SUB:	
		DRW_DrawBMP_FF_Sub( &dobj, interless );	
		break;	

	case DRW_DIM:	
		DRW_DrawBMP_FF_Dim( &dobj, interless );	
		break;	

	case DRW_MUL:	
		DRW_DrawBMP_FF_Mul( &dobj, interless );
		break;	

	case DRW_AMI2:	
		DRW_DrawBMP_FF_Ami( &dobj, interless );	
		break;	

	case DRW_NIS2:	
		DRW_DrawBMP_FF_Nis( &dobj, interless );	
		break;	

	case DRW_NEG:	
		DRW_DrawBMP_FF_Neg(	&dobj, interless );
		break;	

	case DRW_VIV2:
		DRW_DrawBMP_FF_Viv( &dobj, interless );	
		break;	

	case DRW_MOZ2:	
		DRW_DrawBMP_FF_Moz(	&dobj, interless );	
		break;	

	case DRW_BOM2:	
		DRW_DrawBMP_FF_Bom(	&dobj, interless );
		break;	

	case DRW_FLT2:	
		DRW_DrawBMP_FF_Flt( &dobj, interless );
		break;	

	case DRW_RP22:	
		DRW_DrawBMP_FF_Rp2( &dobj );	
		break;	

	case DRW_RPL2:	
		DRW_DrawBMP_FF_RPL( &dobj, interless );	
		break;	

	case DRW_SUI2:	
		DRW_DrawBMP_FF_Sui(	&dobj, interless );
		break;	

	case DRW_SBL2:	
		DRW_DrawBMP_FF_Sbl(	&dobj, interless );	
		break;	

	case DRW_WAV2:	
		DRW_DrawBMP_FF_Wav(	&dobj, interless );	
		break;	

	case DRW_WBL2:	
		DRW_DrawBMP_FF_Wbl(	&dobj, interless );	
		break;	

	case DRW_LST2:	
		DRW_DrawBMP_FF_Lst(	&dobj, interless );	
		break;	

	case DRW_LBL2:	
		DRW_DrawBMP_FF_Lbl(	&dobj, interless );
		break;	

	case DRW_WAP2:	
		DRW_DrawBMP_FF_Wap(	&dobj, interless );	
		break;	

	case DRW_OLY:	
		DRW_DrawBMP_FF_Oly(	&dobj, interless );	
		break;	

	case DRW_CML:	
		DRW_DrawBMP_FF_Cml(	&dobj, interless );	
		break;	

	case DRW_CMD:	
		DRW_DrawBMP_FF_Cmd(	&dobj, interless );
		break;	
...
BOOL DRW_DrawBMP_FB_Gnm( DRAW_OBJECT *dobj, BMP_BT *src )
{
	...
	if( gnm>=256 ) 
		return 1;
	if( gnm<=0   ) 
		return DRW_DrawBMP_FB_Add( dobj, src );	
	for( y=0; y < h ; y++, dest_p+=drev, src_p+=srev )
	{	
		work=0;
		for( x2=0 ; x2 < gnm ; x2++ )
	...
	{			
		for( y=0; y < h ; y++, dest_p+=drev, src_p+=srev )
		{
			for( x=0 ; x < w ; x++, dest_p++, src_p+=xinc )
			{
				if(*src_p!=nuki)
				{
					if(dest_p->b < spal[*src_p].b)
						dest_p->b = spal[*src_p].b;
					if(dest_p->g < spal[*src_p].g) 
						dest_p->g = spal[*src_p].g;
					if(dest_p->r < spal[*src_p].r) 
						dest_p->r = spal[*src_p].r;
				}
			}
		}
	}
...
BOOL DRW_DrawBMP_FB( DRAW_OBJECT dobj, int interless )
{
	...
	if( dest->buf==NULL ) 
	{ 
		DebugPrintf(_T("描画領域がありません[DRW_DrawBMP_FB]\n")); 
		return FALSE; 
	}
	if( src->buf==NULL  ) 
	{	
		DebugPrintf(_T("画像領域がありません[DRW_DrawBMP_FB]\n")); 
		return FALSE; 
	}
	if( dobj.clip!=(void*)-1 )
	{
	...
	else
	{
		...
		{						
			...
			{
			case DRW_SBL2:	
				BMP_ChangePalBright_Full( pal, 128/2, 128/2, 128/2 );	
				break;

			case DRW_BLD2:	
				BMP_ChangePalBright_Full( pal, dobj.dnum/2, dobj.dnum/2, dobj.dnum/2 );	
				break;

			case DRW_NEG:	
				BMP_ChangePalNega_Full( pal );							
				break;	
			}
		}
	}
	...
	case DRW_VIV2:
	case DRW_NML:	
		DRW_DrawBMP_FB_Std( &dobj, &src2 );	
		break;	

	case DRW_BLD2:	
		DRW_DrawBMP_FB_Bld( &dobj, &src2, pal2 );	
		break;	

	case DRW_ADD:	
		DRW_DrawBMP_FB_Add( &dobj, &src2 );	
		break;	

	case DRW_OOI:	
		DRW_DrawBMP_FB_Ooi( &dobj, &src2 );
		break;	

	case DRW_SUB:	
		DRW_DrawBMP_FB_Sub( &dobj, &src2 );
		break;	

	case DRW_MUL:	
		DRW_DrawBMP_FB_Mul( &dobj, &src2 );
		break;	

	case DRW_AMI2:	
		DRW_DrawBMP_FB_Ami( &dobj, &src2 );	
		break;	

	case DRW_NIS2:	
		DRW_DrawBMP_FB_Nis( &dobj, &src2 );
		break;	

	case DRW_BOM2:	
		DRW_DrawBMP_FB_Bom( &dobj, &src2 );	
		break;	

	case DRW_GNM2:	
		DRW_DrawBMP_FB_Gnm( &dobj, &src2 );
		break;	

	case DRW_FLT2:	
		DRW_DrawBMP_FB_Flt( &dobj, &src2 );
		break;	

	case DRW_OLY:	
		DRW_DrawBMP_FB_Oly( &dobj, &src2 );	
		break;	

	case DRW_CML:	
		DRW_DrawBMP_FB_Cml( &dobj, &src2 );	
		break;	

	case DRW_CMD:	
		DRW_DrawBMP_FB_Cmd( &dobj, &src2 );	
		break;	

	case DRW_SUI2:	
		DRW_DrawBMP_FB_Sui(	&dobj, &src2 );
		break;	

	case DRW_SBL2:	
		DRW_DrawBMP_FB_Sbl(	&dobj, &src2 );	
		break;	

	case DRW_RP22:	
		DRW_DrawBMP_FB_Rp2(	&dobj, &src2 );	
		break;	
	}
	return ret;
}
...
BOOL DRW_DrawZOOM_FT_Bld( DRAW_OBJECT *dobj, int dw2, int dh2, int sw2, int sh2, int xxx, int yyy )
{
	...
	if( blnd==BLD_0P  )	
		return 1;															
	if( blnd==BLD_100P)	
		return DRW_DrawZOOM_FT_Std( dobj, dw2, dh2, sw2, sh2, xxx, yyy );	
	px = sw2/dw2 * xinc;
	wx = sw2%dw2;
	py = sh2/dh2 * yinc;
	wy = sh2%dh2;
...
BOOL DRW_DrawZOOM_FT( DRAW_OBJECT dobj, int interless )
{
	...
	if( dest->buf==NULL ) 
	{ 
		DebugPrintf(_T("描画領域がありません[DRW_DrawZOOM_FT]\n")); 
		return FALSE; 
	}
	if( src->buf==NULL  ) 
	{	
		DebugPrintf(_T("画像領域がありません[DRW_DrawZOOM_FT]\n")); 
		return FALSE; 
	}
	if( dobj.clip!=(void*)-1 )
	{
		if( !ClipRectZ( &dobj.s1x, &dobj.s1y, &dobj.s1w, &dobj.s1h,
						...
						&xx, &yy, dest->sx, dest->sy, src->sx, src->sy, dobj.clip ) )	
			return 1;
	}
	if( dobj.dparam==DRW_AMI2 || dobj.dparam==DRW_BLD2 )
	{
	...
	default:
	case DRW_NML:	
		DRW_DrawZOOM_FT_Std( &dobj, dw2, dh2, sw2, sh2, xx, yy );	
		break;	

	case DRW_BLD2:	
		DRW_DrawZOOM_FT_Bld( &dobj, dw2, dh2, sw2, sh2, xx, yy );	
		break;	

	case DRW_ADD:	
		DRW_DrawZOOM_FT_Add( &dobj, dw2, dh2, sw2, sh2, xx, yy );	
		break;	

	case DRW_SUB:	
		DRW_DrawZOOM_FT_Sub( &dobj, dw2, dh2, sw2, sh2, xx, yy );	
		break;	

	case DRW_MUL:	
		DRW_DrawZOOM_FT_Mul( &dobj, dw2, dh2, sw2, sh2, xx, yy );	
		break;	

	case DRW_AMI2:	
		DRW_DrawZOOM_FT_Ami( &dobj, dw2, dh2, sw2, sh2, xx, yy );	
		break;	

	case DRW_NEG:	
		DRW_DrawZOOM_FT_Neg( &dobj, dw2, dh2, sw2, sh2, xx, yy );	
		break;	

	case DRW_VIV2:	
		DRW_DrawZOOM_FT_Viv( &dobj, dw2, dh2, sw2, sh2, xx, yy );	
		break;	
	}
	return TRUE;
}
...
BOOL DRW_DrawZOOM_FF_Bld( DRAW_OBJECT *dobj, int dw2, int dh2, int sw2, int sh2, int xxx, int yyy, int interless )
{
	...
	if( blnd==BLD_0P  )	
		return 1;															
	if( blnd==BLD_100P)	
		return DRW_DrawZOOM_FF_Std( dobj, dw2, dh2, sw2, sh2, xxx, yyy, interless );	
	px = sw2/dw2 * xinc;
	wx = sw2%dw2;
	py = sh2/dh2 * yinc;
	wy = sh2%dh2;
...
BOOL DRW_DrawZOOM_FF( DRAW_OBJECT dobj, int interless )
{
	...
	if( dest->buf==NULL ) 
	{ 
		DebugPrintf(_T("描画領域がありません[DRW_DrawZOOM_FF]\n")); 
		return FALSE; 
	}
	if( src->buf==NULL  ) 
	{	
		DebugPrintf(_T("画像領域がありません[DRW_DrawZOOM_FF]\n")); 
		return FALSE; 
	}
	...
	{
		if( !ClipRectZ( &dobj.s1x, &dobj.s1y, &dobj.s1w, &dobj.s1h,
						&dobj.dx,  &dobj.dy,  &dobj.dw,  &dobj.dh, &xx, &yy, dest->sx, dest->sy, src->sx, src->sy, dobj.clip ) )	
			return 1;
	}
	if( dobj.dparam==DRW_AMI2 || dobj.dparam==DRW_BLD2 )
	{
	...
	default:
	case DRW_NML:	
		DRW_DrawZOOM_FF_Std( &dobj, dw2, dh2, sw2, sh2, xx, yy, interless );	
		break;	

	case DRW_BLD2:	
		DRW_DrawZOOM_FF_Bld( &dobj, dw2, dh2, sw2, sh2, xx, yy, interless );	
		break;	

	case DRW_ADD:	
		DRW_DrawZOOM_FF_Add( &dobj, dw2, dh2, sw2, sh2, xx, yy );	
		break;	

	case DRW_SUB:	
		DRW_DrawZOOM_FF_Sub( &dobj, dw2, dh2, sw2, sh2, xx, yy );	
		break;	

	case DRW_DIM:	
		DRW_DrawZOOM_FF_Dim( &dobj, dw2, dh2, sw2, sh2, xx, yy, interless );
		break;	

	case DRW_MUL:	
		DRW_DrawZOOM_FF_Mul( &dobj, dw2, dh2, sw2, sh2, xx, yy );	
		break;	

	case DRW_AMI2:	
		DRW_DrawZOOM_FF_Ami( &dobj, dw2, dh2, sw2, sh2, xx, yy, interless );
		break;	

	case DRW_NEG:	
		DRW_DrawZOOM_FF_Neg( &dobj, dw2, dh2, sw2, sh2, xx, yy );	
		break;	

	case DRW_VIV2:	
		DRW_DrawZOOM_FF_Viv( &dobj, dw2, dh2, sw2, sh2, xx, yy, interless );	
		break;	
	}
	return TRUE;
}
...
BOOL DRW_DrawZOOM_FB_Bld( DRAW_OBJECT *dobj, int dw2, int dh2, int sw2, int sh2, int xxx, int yyy, BMP_BT *src )
{
	...
	if( blnd==BLD_0P  )	
		return 1;																	
	if( blnd==BLD_100P)	
		return DRW_DrawZOOM_FB_Std( dobj, dw2, dh2, sw2, sh2, xxx, yyy, src );
	px = sw2/dw2 * xinc;
	wx = sw2%dw2;
	py = sh2/dh2 * yinc;
	wy = sh2%dh2;
...
BOOL DRW_DrawZOOM_FB( DRAW_OBJECT dobj, int interless )
{
	...
	if( dest->buf==NULL ) 
	{ 
		DebugPrintf(_T("描画領域がありません[DRW_DrawZOOM_FB]\n")); 
		return FALSE; 
	}
	if( src->buf==NULL  ) 
	{	
		DebugPrintf(_T("画像領域がありません[DRW_DrawZOOM_FB]\n")); 
		return FALSE; 
	}
	if( dobj.clip!=(void*)-1 )
	{
	...
	case DRW_VIV2:	
	case DRW_NML:	
		DRW_DrawZOOM_FB_Std( &dobj, dw2, dh2, sw2, sh2, xx, yy, &src2 );	
		break;	

	case DRW_BLD2:	
		DRW_DrawZOOM_FB_Bld( &dobj, dw2, dh2, sw2, sh2, xx, yy, &src2 );	
		break;	

	case DRW_ADD:	
		DRW_DrawZOOM_FB_Add( &dobj, dw2, dh2, sw2, sh2, xx, yy, &src2 );
		break;	

	case DRW_SUB:	
		DRW_DrawZOOM_FB_Sub( &dobj, dw2, dh2, sw2, sh2, xx, yy, &src2 );	
		break;	

	case DRW_MUL:	
		DRW_DrawZOOM_FB_Mul( &dobj, dw2, dh2, sw2, sh2, xx, yy, &src2 );	
		break;	

	case DRW_AMI2:	
		DRW_DrawZOOM_FB_Ami( &dobj, dw2, dh2, sw2, sh2, xx, yy, &src2 );	
		break;	
	}
	return TRUE;
}
					   ...
					   int r, int g, int b, int param, int blnd )
{
	...
	default:
	case DRW_NML:	
		DRW_DrawXLine_FF_Std( dest_p, src, sx2, sy2, sx/ww, sy/ww, dsx, dsy, fsx, fsy, w, ww, nuki, r, g, b );				
		break;	

	case DRW_BLD2:	
		DRW_DrawXLine_FF_Bld( dest_p, src, sx2, sy2, sx/ww, sy/ww, dsx, dsy, fsx, fsy, w, ww, nuki, r, g, b, blnd );		
		break;

	case DRW_ADD:	
		DRW_DrawXLine_FF_Add( dest_p, src, sx2, sy2, sx/ww, sy/ww, dsx, dsy, fsx, fsy, w, ww, nuki, r, g, b );				
		break;

	case DRW_SUB:	
		DRW_DrawXLine_FF_Sub( dest_p, src, sx2, sy2, sx/ww, sy/ww, dsx, dsy, fsx, fsy, w, ww, nuki, r, g, b );				
		break;

	case DRW_MUL:	
		DRW_DrawXLine_FF_Mul( dest_p, src, sx2, sy2, sx/ww, sy/ww, dsx, dsy, fsx, fsy, w, ww, nuki, r, g, b );				
		break;

	case DRW_AMI2:	
		DRW_DrawXLine_FF_Ami( dest_p, src, sx2, sy2, sx/ww, sy/ww, dsx, dsy, fsx, fsy, w, ww, nuki, r, g, b, blnd,dx1,dy );	
		break;	
...
BOOL DRW_DrawPOLY4_FF( DRAW_OBJECT dobj, int interless )
{
	...
	if( dest->buf==NULL ) 
	{ 
		DebugPrintf(_T("描画領域がありません\n")); 
		ret=FALSE; 
		goto end; 
	}
	if( src->buf==NULL  ) 
	{ 
		DebugPrintf(_T("画像領域がありません\n")); 
		ret=FALSE; 
		goto end; 
	}
	if( dobj.clip==NULL ) 
...
BOOL DRW_DrawXLine_FB( RGB24 *dest_p,int dy, BMP_BT *src, MIN_MAX *mi, MIN_MAX *ma, int nuki, RECT *clip, int param, int blnd )
{
	...
	{ 
		dsx = (-sx)%ww; 
		fsx=-1; 
		sx2 = LIM(sx*xx/ww + mi->sx-1,0,src->sx-1); 
	}
	else	 
	{ 
		dsx =   sx %ww; 
		fsx= 1; 
		sx2 = LIM(sx*xx/ww + mi->sx,  0,src->sx-1); 
	}
	if(sy<0) 
	{ 
		dsy = (-sy)%ww; 
		fsy=-1; 
		sy2 = LIM(sy*xx/ww + mi->sy-1,0,src->sy-1); 
	}
	else	 
	{ 
		dsy =   sy %ww; 
		fsy= 1; 
		sy2 = LIM(sy*xx/ww + mi->sy,  0,src->sy-1); 
	}
	switch( param )
	{
	default:
	...
	case DRW_BLD2:	
		DRW_DrawXLine_FB_Bld( dest_p, src, sx2, sy2, sx/ww, sy/ww, dsx, dsy, fsx, fsy, w, ww, nuki, blnd );			
		break;	
	
	case DRW_ADD:	
		DRW_DrawXLine_FB_Add( dest_p, src, sx2, sy2, sx/ww, sy/ww, dsx, dsy, fsx, fsy, w, ww, nuki );				
		break;	
	
	case DRW_SUB:	
		DRW_DrawXLine_FB_Sub( dest_p, src, sx2, sy2, sx/ww, sy/ww, dsx, dsy, fsx, fsy, w, ww, nuki );				
		break;	
	
	case DRW_MUL:	
		DRW_DrawXLine_FB_Mul( dest_p, src, sx2, sy2, sx/ww, sy/ww, dsx, dsy, fsx, fsy, w, ww, nuki );				
		break;	
	
	case DRW_AMI2:	
		DRW_DrawXLine_FB_Ami( dest_p, src, sx2, sy2, sx/ww, sy/ww, dsx, dsy, fsx, fsy, w, ww, nuki, blnd,dx1,dy );	
		break;
