...
HPALETTE PAL_Initialize( void )
{
	...
	lpUseTbl = (BYTE *)GAlloc( sizeof(char)*256 );
	if( lpUseTbl == NULL )	
		return FALSE ;
	...
	{	
		lpUseTbl[count] = 0xff;	
		lpUseTbl[246+count] = 0xff;	
	}
	lpUseTbl[10] = 0xff;
	CopyMemory( logpal.palPalEntry, lpPalData, sizeof(PALETTEENTRY)*256 );
	if( (hPalette = CreatePalette( (LOGPALETTE*)&logpal )) == NULL )	
		return FALSE;
	return hPalette;
}
...
BOOL PAL_SetEntry( RGB32* rgbPal, int top, int num )
{
	...
	if( rgbPal==NULL )
	{
		char	*buf;
		no = 0;
		PAC_LoadFile( BMP_GetPackDir(), _T("236.act"), &buf );
		for( count=0 ; count<256 ; count++ )
		{
	...
	else
	{
		...
		{
			DebugBox(NULL, _T("パレット設定に失敗しました"));
		}
	}
	return TRUE;
}
...
BOOL PAL_AddEntry( RGB32* rgbPal, int num, LPBYTE lpChgTbl )
{
	int		src, dst;
	char	tbl[256];
	if( (hPalette == NULL) || (lpPalData == NULL) )	
		return FALSE;
