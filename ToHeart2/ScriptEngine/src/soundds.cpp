...
ClSoundDS::ClSoundDS(HWND hWnd,ClReadFile *readFile,BOOL b3DSound)
{
	...
	if(b3DSound) 
		dsbdDesc.dwFlags |= DSBCAPS_CTRL3D;
...
void ClSoundDS::ReleaseSoundBuffer(int handle)
{
	...
	else
	{
		...
		if(lpSoundBuffer->lpNext)
			lpSoundBuffer->lpNext->lpPrev = lpSoundBuffer->lpPrev;
	}
...
ClResult ClSoundDS::ChangeMusic(int &handle,int arcFileNum,int fnum,BOOL bFade,int repeat, int vol )
{
	BOOL			ret;
	ClSoundBuffer	*lpSoundBuffer;
	int				num;	
	...
	{		
		...
		if(status != PCM_STOP)
		{	
			if(lpSoundBuffer->orgMusicNum==num) 
				return Cl_OK;
			if(FALSE==bFade)
			{
				Stop(handle);
	...
	int volume = bgmVolume*vol /10;
	SetVolume(handle,volume,FALSE);
	play(handle,repeat);
	return Cl_OK;
} 

#define BAR_GET_SIZE	4096
...
ClResult ClSoundDS::GetPlayStreamVolume(int &handle, DWORD volme[2] )
{
	...
	{		
		...
		if(status == PCM_PLAY)
		{
			if( lpSoundBuffer->lpDSBuffer->GetCurrentPosition( &CurrentPlayCursor, &CurrentWriteCursor ) == DS_OK )
			{	
				if( lpSoundBuffer->lpDSBuffer->Lock( CurrentWriteCursor, BAR_GET_SIZE, (void **)&lpPtr1, &dwLen1, (void **)&lpPtr2, &dwLen2, 0 ) == DS_OK )
				{
					CopyMemory( buf, lpPtr1, dwLen1 );
					if(lpPtr2)
					{
						CopyMemory( buf+dwLen1, lpPtr2, dwLen2 );
					}
					lpSoundBuffer->lpDSBuffer->Unlock( lpPtr1, dwLen1, lpPtr2, dwLen2 );
					for(i=0;i<(dwLen1+dwLen2)/4;i++)
					{
						volme[0]+=abs(*(short*)(buf+i*4));
						volme[1]+=abs(*(short*)(buf+i*4+2));
					}
					if( (dwLen1+dwLen2)/4 > 0 )
					{
						volme[0] = volme[0]/( (dwLen1+dwLen2)/4 );
						volme[1] = volme[1]/( (dwLen1+dwLen2)/4 );
					}
					else
					{
						volme[0] = 0;
...
ClResult ClSoundDS::playVoice(int &handle,int arcFileNum,char *fname, int repeat, int vol )
{
	...
	int volume = voiceVolume*vol /10;
	SetVolume(handle,volume,FALSE);
	play(handle,repeat);
	return Cl_OK;
} 

ClResult ClSoundDS::CheckVoiceFile(int arcFileNum,char *fname)
{
	char	workBuf[64];
	wsprintf(workBuf,"%s.ogg",fname);
	int num = readFile->SearchFile(arcFileNum,workBuf);
...
void ClSoundDS::NextMusicStart()
{
	...
	while(lpNext)
	{
		...
		if(lpSoundBuffer && lpSoundBuffer->nextMusicNum>=0)
		{
			...
			int volume = bgmVolume*0xff /10;
			SetVolume(handle,volume,FALSE);
			if(TRUE==ret) 
				lpSoundDS->play(handle,nextRepeat);
		}
		lpNext = lpNext->next;
	}
...
ClResult ClSoundDS::FadeOut(int handle)
{
	...
	DWORD			dwThreadID;
	lpSoundBuffer = SelectSoundBuffer(handle);
	if(NULL==lpSoundBuffer || lpSoundBuffer->status==PCM_STOP) 
		return Cl_OK;
...
void ClSoundDS::Stop(int handle)
{
	...
	{	
		...
		while(PCM_STOP!=lpSoundBuffer->status)
			Sleep(1);
	}
	else
	{
...
void ClSoundDS::SetVolume(int handle,int value, int setting_20 )
{
	ClSoundBuffer	*lpSoundBuffer;
	...
	if(value > 0)
	{
		...
		{
			db = LONG(log10(value/255.0f) *40*100);
		}
	}
...
BOOL ClSoundDS::CreateStreamBuffer(int handle,int arcFileNum,int playNum)
{
	...
	{	
		...
		dsbdesc.dwFlags = DSBCAPS_CTRLVOLUME | DSBCAPS_CTRLPOSITIONNOTIFY | DSBCAPS_LOCSOFTWARE | DSBCAPS_GLOBALFOCUS | DSBCAPS_GETCURRENTPOSITION2;
...
BOOL ClSoundDS::CreateDummyBuffer()
{
	...
	dstWF.wFormatTag = WAVE_FORMAT_PCM;
	dstWF.nChannels = 1;
	dstWF.nSamplesPerSec = 48000;
	dstWF.nBlockAlign = 2;
	dstWF.nAvgBytesPerSec = dstWF.nSamplesPerSec * dstWF.nBlockAlign;
	dstWF.wBitsPerSample = 16;
	dstWF.cbSize = 0;
	DSBUFFERDESC	dsbdesc;
	ZeroMemory(&dsbdesc, sizeof(DSBUFFERDESC));
	dsbdesc.dwSize = sizeof(DSBUFFERDESC);
	dsbdesc.dwFlags = DSBCAPS_CTRLVOLUME | DSBCAPS_LOCSOFTWARE | DSBCAPS_GLOBALFOCUS;
	dsbdesc.dwBufferBytes = dstWF.nAvgBytesPerSec /2;	
	dsbdesc.lpwfxFormat = &dstWF;
...
BOOL ClSoundDS::AddSoundBuffer(int handle,char *fileBuf,BOOL b3DSound)
{
	...
	if(pcmFormat.wf.wFormatTag == WAVE_FORMAT_PCM)
	{
		...
		ZeroMemory(&dsbdesc, sizeof(DSBUFFERDESC));
		dsbdesc.dwSize = sizeof(DSBUFFERDESC);
		dsbdesc.dwFlags = DSBCAPS_CTRLVOLUME | DSBCAPS_CTRLPOSITIONNOTIFY | DSBCAPS_LOCSOFTWARE | DSBCAPS_GLOBALFOCUS;
...
oggFilEnd:
						...
						{
							if(lpSoundBuffer->repeat) 
								lpSoundBuffer->repeat--;
							if(lpSoundBuffer->nextStream>=0)
							{	
								int size__;
								lpSoundBuffer->readFile->StreamCloseFile(lpSoundBuffer->arcFileNum,lpSoundBuffer->streamNum);
								lpSoundBuffer->streamNum = lpSoundBuffer->readFile->StreamOpenFileNum(lpSoundBuffer->arcFileNum,lpSoundBuffer->nextStream,size__);
								lpSoundBuffer->musicNum  = lpSoundBuffer->nextStream;
								lpSoundBuffer->nextStream = -1;
							}
				...
				else
				{
					...
					size = lpSoundBuffer->readFile->StreamReadFile(lpSoundBuffer->arcFileNum,lpSoundBuffer->streamNum, (char *)lpSoundBuffer->acmSrc.lpStream,lpSoundBuffer->acmSrc.dwStreamSize);
...
DWORD WINAPI fo_exec(LPVOID param)
{
	...
	for(int i=15;i>0;i--)
	{
		...
		if(bgmVolume > 0)
		{
			db = LONG(log10((bgmVolume/10.0f)*i/16.0f) *40*100);
		}
		lpSoundBuffer->lpDSBuffer->SetVolume(db);
		Sleep(50);
	}
