...
{
	if(BackStruct.flag && bak_no==BackStruct.bno && chg_type==BAK_DIRECT) 
		return ;
	AVG_ResetHalfTone();
...
int AVG_SakuraSakuTH2( int scene )
{
	...
	{	
		if( scene >= 10000) 
			break;		
		if(      ( scene >=  1 && scene <=  4) || scene ==  78) 
			scene =  1;
		else if( ( scene >=  5 && scene <=  8) || scene ==  79) 
			scene =  5;
		else if( ( scene >= 34 && scene <= 37) || scene ==  80) 
			scene = 34;
		else if( ( scene >= 48 && scene <= 51) || scene ==  81) 
			scene = 48;
		else 
			break;
		int m, d;
...
void AVG_SetBack(int bak_no, int x, int y, int chg_type, int char_disp, int cg_flag, int fd_max, int vague )
{
	...
	if(BackStruct.flag && bak_no==BackStruct.bno && chg_type==BAK_DIRECT) 
		return ;
	AVG_ResetBack(0);
	if(BackStruct.tone_back==-1)
		tone = BackStruct.tone_no;		
	else						
		tone = BackStruct.tone_back;	
	BackStruct.tone_no2 = BackStruct.tone_no;
	if(bak_no==-1) 
		return ;
	if(bak_no==-2) 
		bak_no = BackStruct.bno;
	if(bak_no>=0)
	{
		if(cg_flag==1)
		{
		...
		if(back_max) 
		{
			...
			default:
				if( chg_type>=BAK_PATTERN )
				{
					...
					int	rparam=0;
					if(chg_type&BAK_PTF_REV_W)
						rparam |= REV_W;
					if(chg_type&BAK_PTF_REV_H)
						rparam |= REV_H;
		...
		else
		{
			...
			case BAK_CFADE:
				if(sp)	
					...
					DSP_SetGraphParam( GRP_BACK, DRW_AMI(rate) );
				break;

			case BAK_CFADE_UP:
				if(sp)	
					DSP_SetGraphParam( GRP_BACK, DRW_LCF(UP,rate) );
				else	
					DSP_SetGraphParam( GRP_BACK, DRW_LPP(UP,rate) );
				break;

			case BAK_CFADE_DO:
				if(sp)	
					DSP_SetGraphParam( GRP_BACK, DRW_LCF(DO,rate) );
				else	
					DSP_SetGraphParam( GRP_BACK, DRW_LPP(DO,rate) );
				break;

			case BAK_CFADE_RI:
				if(sp)	
					DSP_SetGraphParam( GRP_BACK, DRW_LCF(RI,rate) );
				else	
					DSP_SetGraphParam( GRP_BACK, DRW_LPP(RI,rate) );
				break;

			case BAK_CFADE_LE:
				if(sp)	
					DSP_SetGraphParam( GRP_BACK, DRW_LCF(LE,rate) );
				else	
					DSP_SetGraphParam( GRP_BACK, DRW_LPP(LE,rate) );
				break;

			case BAK_CFADE_CE:
				if(sp)	
					DSP_SetGraphParam( GRP_BACK, DRW_LCF(CE,rate) );
				else	
					DSP_SetGraphParam( GRP_BACK, DRW_LPP(CE,rate) );
				break;

			case BAK_CFADE_OU:
				if(sp)	
					DSP_SetGraphParam( GRP_BACK, DRW_LCF(OU,rate) );
				else	
					DSP_SetGraphParam( GRP_BACK, DRW_LPP(OU,rate) );
				break;

			case BAK_DIA1:	case BAK_DIA2:	case BAK_DIA3:
				if(sp)	
					DSP_SetGraphParam( GRP_BACK, DRW_DIO(BackStruct.fd_type-BAK_DIA1,rate) );
				else	
					DSP_SetGraphParam( GRP_BACK, DRW_DIA(BackStruct.fd_type-BAK_DIA1,rate) );
				break;
			...
			case BAK_CFZOOM1:
				...
				if(sp && MainWindow.draw_mode2!=16)	
					DSP_SetGraphParam( GRP_BACK, DRW_BLD(128-rate/2) );
				else								
					DSP_SetGraphParam( GRP_BACK, DRW_AMI(256-rate)   );
...
void AVG_SetBackFadePatern( int in_out, int add_sub, int pno, int fade, int rev, int kaiten, int rx, int ry )
{
	...
	if( !Avg.level )
	{
		...
		{
			if(add_sub)	
				AVG_SetBackFade( 255, 255, 255, fade );
			else		
				AVG_SetBackFade(   0,   0,   0, fade );
		}
		return ;
	}
	BackStruct.br_flag = 1;		
	...
	if(rev)	
		DSP_LoadBmp( BMP_WORK+10, BMP_256P, fname, "nega.amp" );
	else	
		DSP_LoadBmp( BMP_WORK+10, BMP_256P, fname );
	DSP_SetGraph( GRP_WORK+20, BMP_WORK+10, LAY_FORE+3, ON, CHK_NO );
	if(add_sub)	
		DSP_SetGraphParam( GRP_WORK+20, DRW_ADD );
	else		
		DSP_SetGraphParam( GRP_WORK+20, DRW_SUB );
	if(!DemoFede)
	{
	...
	{	
		...
		else
		{
			...
			else
			{
				...
				{
				case 0x01:	
					rate = 255-255*BackStruct.br_cnt/back_max;	
					break;	
				
				case 0x00:	
					rate = 255*BackStruct.br_cnt/back_max;		
					break;
...
void AVG_CopyBack( int sc )
{
	if(BackStruct.flag)
	{
		if(sc)
		{
			...
			{
			case 0:	
				AVG_SetBackPos( BackStruct.sx, BackStruct.sy );											
				break;
			
			case 1:	
				AVG_SetBackPosZoom( BackStruct.sx, BackStruct.sy, BackStruct.sw, BackStruct.sh );		
				break;
			
			case 2:	
				AVG_SetBackPosPoly( BackStruct.sx,  BackStruct.sy,  BackStruct.sx2, BackStruct.sy2,
										BackStruct.sx3, BackStruct.sy3, BackStruct.sx4, BackStruct.sy4 );	
				break;
			}
		}
		else
		{
			...
			case 1:	
				AVG_SetBackPosZoom( BackStruct.x, BackStruct.y, BackStruct.w, BackStruct.h );		
				break;

			case 2:	AVG_SetBackPosPoly( BackStruct.x,  BackStruct.y,  BackStruct.x2, BackStruct.y2,
										BackStruct.x3, BackStruct.y3, BackStruct.x4, BackStruct.y4 );	
				break;
			}
		}	
		DSP_CopyBmp( BMP_BACK, BMP_BACK2 );
	...
	if(BackStruct.sc_flag)
	{
		...
		else
		{
			...
			case 1:
				...
				{
				case 16:	
					sx = ((BMP_H*)bmp)->sx;	sy = ((BMP_H*)bmp)->sy;	
					break;
				
				case 24:	
					sx = ((BMP_F*)bmp)->sx;	sy = ((BMP_F*)bmp)->sy;	
					break;
				
				case 32:	
					sx = ((BMP_T*)bmp)->sx;	sy = ((BMP_T*)bmp)->sy;	
					break;
				}
...
void AVG_ControlBackCScope(void)
{
	...
	if(BackStruct.cs_flag && BackStruct.cs_type!=-1)
	{
		...
		else
		{
			if(BackStruct.cs_type)
			{	
				h = 64*(BackStruct.cs_cnt+1)/back_max;		
			}
			else				  
			{	
				h = 64-64*(BackStruct.cs_cnt+1)/back_max;	
			}
			BackStruct.cs_clip.x = 0;
...
{
	if( BackStruct.sk_speed==0 ) 
		return 0;
	if( BackStruct.sk_flag ==0 ) 
		return 0;
	return 1;
}
		...
		else
		{
			...
			case SHAKE_ALL_SIN_SET:
				...
				if(back_max)
				{
					...
					{
					case DIR_L: case DIR_DL: case DIR_UL:	
						x =-COS(cnt2)*pich/512/8;	
						break;
					...
					case DIR_R: case DIR_DR: case DIR_UR:	
						x = COS(cnt2)*pich/512/8;	
						break;
					}
					switch(BackStruct.sk_dir)
					{
					case DIR_D: case DIR_DL: case DIR_DR:	
						y = COS(cnt2)*pich/512/8;	
						break;
					...
					case DIR_U: case DIR_UL: case DIR_UR:	
						y =-COS(cnt2)*pich/512/8;	
						break;
					}
...
void AVG_ControlRipple( void )
{
	...
	if( Ripple.flag )
	{
		if(Ripple.angle  < Ripple.tangle  )	
			Ripple.angle  = LIM( Ripple.angle + 1, 0, 255 );
		if(Ripple.angle  > Ripple.tangle  )	
			Ripple.angle  = LIM( Ripple.angle - 1, 0, 255 );
		if(Ripple.fade  < Ripple.tfade  )	
			Ripple.fade  = LIM( Ripple.fade + 1, 0, 255 );
		if(Ripple.fade  > Ripple.tfade  )	
			Ripple.fade  = LIM( Ripple.fade - 1, 0, 255 );
		angle = Ripple.angle;
...
void AVG_LoadBmp( int s_bno, int bmp_bit, TCHAR *fname, int layer, int nuki, int tone_type, TCHAR *tfname, int viv, int bright, TCHAR *pack )
{
	int		tone_no;
	char	*eff_fname=NULL;
	AVG_ReleaseBmp(s_bno);
	...
	if(tfname)	
		_mbscpy( (BYTE*)SpriteBmp[s_bno].tfname, (BYTE*)tfname );
	if(pack)	
		_mbscpy( (BYTE*)SpriteBmp[s_bno].pack, (BYTE*)pack );
	else		
		_mbscpy( (BYTE*)SpriteBmp[s_bno].pack, (BYTE*)PAC_GRP );
...
void AVG_LoadBmpAnime( int s_bno, int bmp_bit, TCHAR *fname, int loop, int layer, int nuki )
{
	int	mh;
	AVG_ReleaseBmp(s_bno);
	if(s_bno!=-1)
...
BOOL AVG_WaitBmpAnime( int s_bno )
{
	...
	if( !SpriteBmp[s_bno].flag ) 
		return 1;
	if( DSP_GetBmpAnimeLoop( SpriteBmp[s_bno].mh )==0 ) 
		return 1; 
...
void AVG_SetAvi( int avino, int bno, int w, int h, TCHAR *fname )
{
	AVG_ResetAvi( avino );
