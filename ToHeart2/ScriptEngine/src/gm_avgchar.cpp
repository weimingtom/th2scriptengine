...
*/
	if( index!=MAX_CHAR )
	{
		...
		{
			if( CharStruct[index].pose!=pose )	
				AVG_SetCharPose( char_no, pose, CHAR_TYPE_CFADE, frame );
		}
	}
	else
	{
...
/*	if( !CheckCharFlag() )
	{
		if( BackStruct.flag )
		{
			...
			{
				if(out_type==-1)	
					AVG_SetBackCScope( OFF, 0 );
				else				
					AVG_SetBackCScope( OFF, frame );
			}
		}
	}
*/
	...
	for( i=0; i<MAX_CHAR ;i++ )
	{
		...
		if(CharStruct[i].flag)
		{
			...
			case CHAR_COND_IN:		
				if(max)
				{
					...
					case CHAR_TYPE_MOVE+1:
						...
						max = max*max*max;
						cnt = max-cnt;
						if(CharStruct[i].type==CHAR_TYPE_MOVE) 
						{	
							locate = -600;	
						}
						else									
			...
			case CHAR_COND_POSE:	
				if(max)
				{
					...
					else
					{
						...
						{
							if(ami)	
								DSP_SetGraphParam2( GRP_CHAR+i, DRW_AMI(rate) );
							else	
								DSP_SetGraphParam2( GRP_CHAR+i, DRW_LPP(UP,rate) );
						}
					}
				}
				break;
			...
			case CHAR_COND_BRIGHT:	
				if(max)	
				{	
					rate = (CharStruct[i].fade2*(max-cnt)+CharStruct[i].fade1*cnt)/max;	
				}
				else	
				{	
					rate =  CharStruct[i].fade1;										
				}
				bright = rate;
				break;
			...
			case CHAR_COND_ALPHA:	
				if(max)	
				{	
					rate = (CharStruct[i].alph2*(max-cnt)+CharStruct[i].alph1*cnt)/max;	
				}
				else	
				{	
					rate =  CharStruct[i].alph1;										
				}
				param = DRW_BLD(rate);
				break;
			}
