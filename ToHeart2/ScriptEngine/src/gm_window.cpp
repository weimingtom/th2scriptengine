...
void GWIN_SetSaveLoadWindow( int type, int bmp_set, int grp_set, int txt_set, int grp_bak, int txt_bak, int mus_lay, int mus_bak )
{
	...
	if(type==SW_SAVE_MODE)	
		DSP_LoadBmp( BmpSetting,    BMP_FULL, "sys0200.tga" );
	else					
		DSP_LoadBmp( BmpSetting,    BMP_FULL, "sys0300.tga" );
	...
	if(type==SW_SAVE_MODE)	
		DSP_LoadBmp( BmpSetting+7,  BMP_TRUE, "sys0250.tga" );
	else					
		DSP_LoadBmp( BmpSetting+7,  BMP_TRUE, "sys0350.tga" );
...
void GWIN_ResetSaveLoadWindow( void )
{
	...
	SaveWindow.flag=0;
	for(i=0;i<70;i++)	
		DSP_ResetGraph( GrpSetting+i );
	for(i=0;i<10;i++)	
		DSP_ReleaseBmp( BmpSetting+i );
...
int GWIN_ControlSaveLoadWindow(void)
{
	...
	if( SaveWindow.flag )
	{
		...
		case SAV_LOAD_CHECK:
			...
			if(GameKey.l)
			{	
				MUS_SetMousePosRect( MainWindow.hwnd, MouseLayer+1, 0 ); 
			}
			if(GameKey.r)
			{	
				MUS_SetMousePosRect( MainWindow.hwnd, MouseLayer+1, 1 ); 
			}
		...
		case SAV_NORMAL:
			...
			if(select!=-1)
			{
				...
				else
				{
					...
					{
					case 10:	
						DSP_SetGraphSMove( GrpSetting+15, 0,    64 );	
						break;

					case 11:	
						DSP_SetGraphSMove( GrpSetting+16, 0, 32+64 );	
						break;

					case 12:	
						DSP_SetGraphSMove( GrpSetting+17, 188, 0 );		
						break;
					}
		...
		case SIDE_NORMAL:
			...
			if(select!=-1)
			{
				...
				if( click )
				{
					...
					{
					case 0:
						if( MainWindow.full_screen == 0 && select%5==1 ) 
							break;
						if( MainWindow.full_screen != 0 && select%5==0 ) 
							break;
					...
					case 2:
						...
						default:
						case 0:	
							SendMessage( (HWND)MainWindow.hwnd, WM_COMMAND, ID_EFFECT_CUT, 0 );	
							break;

						case 1:	
							SendMessage( (HWND)MainWindow.hwnd, WM_COMMAND, ID_EFFECT_HIGH, 0 );	
							break;

						case 2:	
							SendMessage( (HWND)MainWindow.hwnd, WM_COMMAND, ID_EFFECT_NORMAL, 0 );	
							break;

						case 3:	
							SendMessage( (HWND)MainWindow.hwnd, WM_COMMAND, ID_EFFECT_LOW, 0 );	
							break;
						}
						break;
					...
					case 3:
						...
						case 1:	
							AVG_PlayBGMEx( 24, 0, TRUE, 255, FALSE, FALSE );	
							break;

						case 2:	
							SendMessage( (HWND)MainWindow.hwnd, WM_COMMAND, ID_BGM_CHECK, 0 );	
							break;
						}
						break;
					...
					case 4:
						...
						case 1:	
							AVG_PlaySE2( 0, 8025, 0, FALSE, 255 );	
							break;

						case 2:	
							SendMessage( (HWND)MainWindow.hwnd, WM_COMMAND, ID_SE_CHECK, 0 );	
							break;
						}
						break;
					...
					case 5:
						...
						case 1:	
							AVG_SizukuPlayVoice( 0, 1803, -1, -1, 256, FALSE,TRUE );	
							break;

						case 2:	
							SendMessage( (HWND)MainWindow.hwnd, WM_COMMAND, ID_VOICE_CHECK, 0 );	
							break;
						}
						break;
					...
					case 8:
						...
						default:
						case 1:	
							SendMessage( (HWND)MainWindow.hwnd, WM_COMMAND, ID_TEXT_CUT, 0 );		
							break;

						case 2:	
							SendMessage( (HWND)MainWindow.hwnd, WM_COMMAND, ID_TEXT_HIGH, 0 );		
							break;

						case 3:	
							SendMessage( (HWND)MainWindow.hwnd, WM_COMMAND, ID_TEXT_NORMAL, 0 );	
							break;

						case 4:	
							SendMessage( (HWND)MainWindow.hwnd, WM_COMMAND, ID_TEXT_LOW, 0 );		
							break;
						}
						break;
					...
					case 9:
						...
						default:
						case 0:	
							SendMessage( (HWND)MainWindow.hwnd, WM_COMMAND, ID_SKIP_READ, 0 );	
							break;

						case 1:	
							SendMessage( (HWND)MainWindow.hwnd, WM_COMMAND, ID_SKIP_NOREAD, 0 );	
							break;
						}
						break;
...
static void GWIN_InitSettingBar( void )
{
	...
	{
		case 0:	
			SettingWindow.backup[3] = SettingWindow.select[3] = 0;	
			break;
		
		case 1:	
			SettingWindow.backup[3] = SettingWindow.select[3] = 2;	
			break;
		
		case 2:	
			SettingWindow.backup[3] = SettingWindow.select[3] = 1;	
			break;
	}
	...
	{
	case 0:	
		SettingWindow.backup[7] = SettingWindow.select[7] = 2;	
		break;
	
	case 1:	
		SettingWindow.backup[7] = SettingWindow.select[7] = 1;	
		break;
	
	case 2:	
		SettingWindow.backup[7] = SettingWindow.select[7] = 0;	
		break;
	}
...
static void GWIN_SetSettingBar( int gno, int num, int light, int select, int backup, int cnt )
{
	...
	for(i=0;i<num;i++, gno++)
	{
		...
		if(select==i)		
		{	
			DSP_SetGraphParam( gno, DRW_BLD(cnt) );		
		}
		else if(backup==i)	
		{	
			DSP_SetGraphParam( gno, DRW_BLD(256-cnt) );	
		}
		else				
		{	
			DSP_SetGraphDisp( gno, OFF );				
		}
	}
}
