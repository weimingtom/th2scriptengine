...
void AVG_SetNovelMessage( TCHAR *str, int add_flag )
{
	char	buf[1024];
	AVG_SetName( buf, str );	
...
void ControlHistorySystem( void )
{
	...
	if( NovelMessage.disp && !Avg.demo )
	{
		...
		case 1:
			if( click )	
				DSP_SetGraphSMove( GRP_HISTORY+2, HistorySystemSrcX[1]+HistorySystemSrcW[1]*3, HistorySystemSrcY[1] );
			else		
				DSP_SetGraphSMove( GRP_HISTORY+2, HistorySystemSrcX[1]+HistorySystemSrcW[1]*2, HistorySystemSrcY[1] );
			break;

		case 2:
			if( click )	
				DSP_SetGraphSMove( GRP_HISTORY+3, HistorySystemSrcX[2]+HistorySystemSrcW[2]*3, HistorySystemSrcY[2] );
			else		
				DSP_SetGraphSMove( GRP_HISTORY+3, HistorySystemSrcX[2]+HistorySystemSrcW[2]*2, HistorySystemSrcY[2] );
			break;
		...
		case 8:
			...
			{
				if( click )	
					DSP_SetGraphSMove( GRP_HISTORY+command+1, HistorySystemSrcX[command]+HistorySystemSrcW[command]*3, HistorySystemSrcY[command] );
				else		
					DSP_SetGraphSMove( GRP_HISTORY+command+1, HistorySystemSrcX[command]+HistorySystemSrcW[command]*2, HistorySystemSrcY[command] );
			}
			break;
		...
		case 5:
			if( NovelMessage.step1==MSG_WAIT || NovelMessage.step1==MSG_STOP || NovelMessage.step1==MSG_DISP || NovelMessage.step1==MSG_NEXT  )
			{
				if( click )	
					...
					DSP_SetGraphSMove( GRP_HISTORY+6, HistorySystemSrcX[5]+HistorySystemSrcW[5]*2, HistorySystemSrcY[5] );
			}
			break;
			...
			if( (Avg.msg_cut_optin&1) || AVG_CheckScenarioFlag() )
			{
				if( NovelMessage.step1==MSG_WAIT || NovelMessage.step1==MSG_STOP || NovelMessage.step1==MSG_DISP || NovelMessage.step1==MSG_NEXT  )
				{
					if( click )	
						...
						DSP_SetGraphSMove( GRP_HISTORY+7, HistorySystemSrcX[6]+HistorySystemSrcW[6]*2, HistorySystemSrcY[6] );
				}
			}
			break;
...
void AVG_SetWindow( int direct )
{
	...
	if( direct)
	{
		...
		{
			for(i=0;i<3;i++)
				DSP_SetGraph( GRP_WINDOW+i, BMP_WINDOW, LAY_WINDOW,   ON, CHK_ANTI );
		}
		else
		{
			for(i=0;i<3;i++)
				DSP_SetGraph( GRP_WINDOW+i, BMP_WINDOW, LAY_WINDOW,   ON, CHK_256 );
		}
		for(i=3;i<8;i++)
			DSP_SetGraph( GRP_WINDOW+i, BMP_WINDOW, LAY_WINDOW+1, ON, CHK_NO );
		switch(Avg.win_bld)	
		{
		case 0:	
			DSP_SetGraphParam( GRP_WINDOW, DRW_NML );				
			break;
		
		case 1:	
			DSP_SetGraphParam( GRP_WINDOW, DRW_BLD(WIN_ALPHA) );	
			break;

		case 2:	
			DSP_SetGraphParam( GRP_WINDOW, DRW_AMI(128) );			
			break;
		}
		for(i=0;i<8;i++)
	...
	else
	{
		...
		for(i=0;i<3;i++)
			DSP_SetGraph( GRP_WINDOW+i, BMP_WINDOW, LAY_WINDOW,   ON, CHK_256 );
		for(i=3;i<8;i++)
			DSP_SetGraph( GRP_WINDOW+i, BMP_WINDOW, LAY_WINDOW+1, ON, CHK_256 );
		switch(Avg.win_bld)	
		{
		case 0:	
			DSP_SetGraphParam( GRP_WINDOW, DRW_NML );				
...
static void SetMessageWindowEffect( int cnt )
{
	...
	if(Avg.level)
	{
		if(Avg.win_bld)	
		{ 
			DSP_SetGraphParam( GRP_WINDOW, DRW_BLD(WIN_ALPHA*cnt/256) );	
		}
		else			
		{ 
			DSP_SetGraphParam( GRP_WINDOW, DRW_BLD(cnt) );				
		}
		for(i=1;i<8;i++)	
			DSP_SetGraphParam( GRP_WINDOW+i, DRW_BLD(cnt) );
		cnt = cnt*cnt/256;
		cnt = 256-cnt;
		for(i=0;i<8;i++)
	...
	else
	{
		if(Avg.win_bld)	
		{ 
			DSP_SetGraphParam( GRP_WINDOW, DRW_AMI(WIN_ALPHA*cnt/256) );	
		}
		else			
		{ 
			DSP_SetGraphParam( GRP_WINDOW, DRW_AMI(cnt) );				
		}
		for(i=1;i<8;i++)	
			DSP_SetGraphParam( GRP_WINDOW+i, DRW_AMI(cnt));
	}
/*	x = -64*cnt/256;
...
void AVG_ControlWindow( void )
{
	...
	if( Message.wflag )
	{
		...
		case MWIN_STOP:
			...
			for(i=1;i<8;i++)	
				DSP_SetGraphParam( GRP_WINDOW+i, DRW_NML );
			if(Avg.level && Avg.win_bld==1)
			{
				for(i=0;i<3;i++)
					DSP_SetGraphNuki( GRP_WINDOW+i, CHK_ANTI );
			}
			else
			{
				for(i=0;i<3;i++)DSP_SetGraphNuki( GRP_WINDOW+i, CHK_256 );
...
void AVG_SetText( int txt_no, TCHAR *str )
{
...
void AVG_SetMessage( int char_no, TCHAR *str, int type, int add_flag, int dy, int font )
{
	int		i,x;
	if(Message.wflag)
	{
	...
	if( ChipBackStruct.flag && !Message.wflag )
	{
		...
		x = (f-i)*CHIP_FONT/2/2;
		if(font==-1)
			DSP_SetText( TXT_WINDOW, LAY_WINDOW+1, CHIP_FONT, 1, str );
		else		
			DSP_SetText( TXT_WINDOW, LAY_WINDOW+1, font, 1, str );
		if(dy==-1)	
			DSP_SetTextPos( TXT_WINDOW, MSG_CTEXT_X+x, MSG_CTEXT_Y, MSG_CTEXT_W, MSG_CTEXT_H );
		else		
			DSP_SetTextPos( TXT_WINDOW, MSG_CTEXT_X+x, dy, MSG_CTEXT_W, MSG_CTEXT_H );
	...
	{
		if(font==-1)
			DSP_SetText( TXT_WINDOW, LAY_WINDOW+1, SYS_FONT, 1, str );
		else		
			DSP_SetText( TXT_WINDOW, LAY_WINDOW+1, font, 1, str );
		if(dy==-1)
		{
	...
	if(Message.mflag )
	{
		...
		else
		{
			...
			case MSG_WAIT2:
				...
				{
				case 0:	
					DSP_SetGraphSMove( GRP_WINDOW+3, MsgCons[3].sx+64, MsgCons[3].sy );	
					break;
				...
				case 4:	
					DSP_SetGraphSMove( GRP_WINDOW+7, MsgCons[7].sx+10, MsgCons[7].sy );	
					break;
				...
				if( click )
				{
					...
					case 5:	
						if(Message.bmax!=1)
						{
							...
							MUS_GetMousePos( &mx, &my );
							if( my > Message.bar_y )
								AVG_SetLogMessage( Message.bcount+4 );
							else					
								AVG_SetLogMessage( Message.bcount-4 );
							SetDragBar();
							if(Message.mstep!=MSG_LOG)
...
void AVG_SetSelectMessage( TCHAR *mes, int game_flag_no, int game_flag_value, int mes_wait )
{
	if(mes_wait)
...
void AVG_SetSelectMessageEx( TCHAR *mes, TCHAR *sno, int game_flag_no, int game_flag_value )
{
	SelectWindow.ex = 1;
...
void AVG_ControlSelectWindow( void )
{
	...
	if( SelectWindow.flag && SelectWindow.disp && !AVG_GetLaodFlag() )
	{
		...
		case 0:
			if(GameKey.u)	
				MUS_SetMousePosRect( MainWindow.hwnd, 0, (select<0)? 16 : select+16-1 );
			if(GameKey.d)	
				MUS_SetMousePosRect( MainWindow.hwnd, 0, (select<0)? 16 : select+16+1 );
