						...
						int blnd )
{
	...
	if(blnd==BLD_0P )	
		return TRUE;											
	if(blnd==BLD_100P)	
		return PRM_DrawRect_F_Std( dest, dx, dy, w, h, r,g,b );	
	if( MMX_Flag )
	{
		int		w2 = w*3;
		int		w3 = w2/24;
					...
					DWORD wparam )
{
	...
	if( dest->buf==NULL ) 
	{ 
		DebugPrintf(_T("描画領域がありません[PRM_DrawRect_F]\n")); 
		return FALSE; 
	}
	return ret;
}
					...
					DWORD wparam )
{
	...
	default:
	case DRW_NML:	
		PRM_DrawRect_F_Std( dest, dx, dy, w, h, r, g, b );			
		break;	

	case DRW_BLD2:	
		PRM_DrawRect_F_Bld( dest, dx, dy, w, h, r, g, b, blnd );	
		break;	

	case DRW_ADD:	
		PRM_DrawRect_F_Add( dest, dx, dy, w, h, r, g, b );			
		break;	

	case DRW_SUB:	
		PRM_DrawRect_F_Sub( dest, dx, dy, w, h, r, g, b );			
		break;	

	case DRW_MUL:	
		PRM_DrawRect_F_Mul( dest, dx, dy, w, h, r, g, b );			
		break;	

	case DRW_AMI2:	
		PRM_DrawRect_F_Ami( dest, dx, dy, w, h, r, g, b, blnd );	
		break;	

	case DRW_NIS2:	
		PRM_DrawRect_F_Nis( dest, dx, dy, w, h, r, g, b, blnd );	
		break;	
...
BOOL PRM_DrawFxLine_F_Bld( char *dest_p, MIN_MAX *mi, MIN_MAX *ma, RECT *clip, char *rgb_tbl, int blnd )
{
	...
	if(blnd>=256) 
		return PRM_DrawFxLine_F_Std( dest_p, mi, ma, clip, rgb_tbl );
	if(blnd<=0) 
		return TRUE;
	if( mi->x == DispWidth )	
		return TRUE;	
									...
									DWORD wparam )
{
	...
	if( dest->buf==NULL ) 
	{ 
		DebugPrintf(_T("描画領域がありません[PRM_DrawFLine_F]\n")); 
		return FALSE; 
	}
	...
	default:
	case DRW_NML:	
		ret = PRM_DrawFLine_F_Std( dest, x1, y1, x2, y2, r, g, b );			
		break;	

	case DRW_BLD2:	
		ret = PRM_DrawFLine_F_Bld( dest, x1, y1, x2, y2, r, g, b, blnd );	
		break;	

	case DRW_ADD:	
		ret = PRM_DrawFLine_F_Add( dest, x1, y1, x2, y2, r, g, b );			
		break;	

	case DRW_SUB:	
		ret = PRM_DrawFLine_F_Sub( dest, x1, y1, x2, y2, r, g, b );			
		break;	

	case DRW_MUL:	
		ret = PRM_DrawFLine_F_Mul( dest, x1, y1, x2, y2, r, g, b );			
		break;	
									...
									RECT *clip, DWORD wparam )
{
	...
	if( dest->buf==NULL ) 
	{ 
		DebugPrintf(_T("描画領域がありません[PRM_DrawGLine_F]\n")); 
		return FALSE; 
	}
	...
	default:
	case DRW_NML:	
		ret = PRM_DrawGLine_F_Std( dest, x1, y1, r1, g1, b1, x2, y2, r2, g2, b2 );			
		break;	

	case DRW_BLD2:	
		ret = PRM_DrawGLine_F_Bld( dest, x1, y1, r1, g1, b1, x2, y2, r2, g2, b2, blnd );	
		break;

	case DRW_ADD:	
		ret = PRM_DrawGLine_F_Add( dest, x1, y1, r1, g1, b1, x2, y2, r2, g2, b2 );			
		break;

	case DRW_SUB:	
		ret = PRM_DrawGLine_F_Sub( dest, x1, y1, r1, g1, b1, x2, y2, r2, g2, b2 );			
		break;	

	case DRW_MUL:	
		ret = PRM_DrawGLine_F_Mul( dest, x1, y1, r1, g1, b1, x2, y2, r2, g2, b2 );			
		break;	
...
BOOL PRM_DrawHanabi_F( BMP_F *dest,	int cx, int cy, int rate, RECT *clip )
{
	...
	if( dest->buf==NULL ) 
	{ 
		DebugPrintf(_T("描画領域がありません[PRM_DrawHanabi_F]\n")); 
		return FALSE; 
	}
	if( rate==0 ) 
		return 1;
