...
static TCHAR *PackBmpDir = NULL;
static char	HighColorType = BIT16_555;

void BMP_SetPackDir( TCHAR *pac_dir )
{
	PackBmpDir = pac_dir;
}

TCHAR *BMP_GetPackDir( void )
{
	return PackBmpDir;
}
...
BOOL BMP_CheckFile( TCHAR *fname )
{
	return PAC_CheckFile( PackBmpDir, fname );
}

BOOL BMP_PalLoad_act( RGB32 *pal, TCHAR *fname )
{
	int		i,size;
	char	*buf=NULL;
	if( !(size=PAC_LoadFile( PackBmpDir, fname, &buf)) ) 
		return FALSE;
	if( size>768 )
	{
		GFree(buf);
...
BOOL BMP_PalLoad_BT( BMP_BT *bmp_b, TCHAR *fname )
{
	return BMP_PalLoad_act( bmp_b->pal, fname );
}

BOOL BMP_PalSave( RGB32 *pal, TCHAR *fname )
{
	int		i;
	char	buf[256*3];
	for(i=0; i<256 ;i++)
...
BOOL BMP_PalSave_BT( BMP_BT *bmp_b, TCHAR *fname )
{
...
int BMP_LoadTonecurve( TCHAR *fname )
{
	int		size;
	GFree(ToneCourve);
	...
	{
		DebugPrintf(_T("トーンカーブファイル[%s]がありません"), fname );
		return 0;
	}
	if(size!=1280 && size!=768 && size!=256)
	{
		GFree(ToneCourve);
		DebugBox( NULL, _T("サイズが違います。本当にトーンカーブファイルかチェックしてください[%s]"), fname );
		return 0;
	}
	return size;
}
...
BOOL BMP_SetTonecurve_T( BMP_T *bmp_t, TCHAR *fname, int viv )
{
	int		i;
	char	*rgb, *r, *g, *b;
...
BOOL BMP_SetTonecurve_F( BMP_F *bmp_f, TCHAR *fname, int viv )
{
	int		i;
	char	*rgb, *r, *g, *b;
	int		size = BMP_LoadTonecurve( fname );
...
BOOL BMP_SetTonecurve_H( BMP_H *bmp_h, TCHAR *fname, int viv )
{
	int		i;
	char	*rgb, *r, *g, *b;
	int		size = BMP_LoadTonecurve( fname );
...
BOOL BMP_SetTonecurve_BT( BMP_BT *bmp_bt, TCHAR *fname, int viv )
{
	int		i;
	char	*rgb, *r, *g, *b;
	int		size = BMP_LoadTonecurve( fname );
...
BOOL BMP_MakeBMP_F2F_BJR( BMP_F *bmp_f, char *ptr, int rev_bmp, TCHAR *fname )
{
	int		x,y;
	int		w=bmp_f->sx, h=bmp_f->sy;
	int		key1,key2,key3;
...
BOOL BMP_LoadBMP_X2X( TCHAR *fname, BMP_SET *bmp_set, TCHAR *pack )
{
	...
	if(pack==NULL) 
		pack = PackBmpDir;
	if( !PAC_LoadFile( pack, fname, &buf) ) 
		return FALSE;
	ptr = buf;
	...
	else if( _mbsicmp( (BYTE *)STD_GetAttr(fname), (BYTE *)"tga")==0 )
	{
		...
		{
			DebugBox(NULL, _T("%sの読み込みに失敗しました"), fname);
			GlobalFree( buf );
			return 0;
		}
		bmp_set->flag = TRUE;
	...
	{	
		...
		if( 0>BMPFileInfo->biHeight )
		{	
			BMPFileInfo->biHeight*=-1;	
			rev_bmp=0;	
		}
		...
		{
		default:	
			DebugBox( NULL, _T("BMPデータのBIT数が異常です[%s](%dBIT?)"), fname, BMPFileInfo->biBitCount );	
			break;
		...
		case 16:
			DebugBox(NULL, _T("16BITハイカラーの読み込みは非対応です"));
			break;
...
BOOL BMP_LoadBMP_X2T( TCHAR *fname, BMP_T *bmp_t, POINT *pos, POINT *size, int *alp_flag, TCHAR *pack )
{
	BITMAPFILEHEADER    *BMPFileHead;
    BITMAPINFOHEADER    *BMPFileInfo;
	char				*buf, *ptr;
	...
	if( !PAC_LoadFile( pack, fname, &buf) ) 
		return FALSE;
	ptr = buf;
	BMPFileHead = (BITMAPFILEHEADER*)ptr;	
	ptr += sizeof( BITMAPFILEHEADER);	
	BMPFileInfo = (BITMAPINFOHEADER*)ptr;	
	...
	case 8:		
		ret=BMP_MakeBMP_B2T( bmp_t, ptr, rev_bmp, *alp_flag, palnum );	
		break;
	
	case 24:	
		ptr += palnum;	
		ret=BMP_MakeBMP_F2T( bmp_t, ptr, rev_bmp );						
		break;
	
	case 32:	
		ptr += palnum;	
		ret=BMP_MakeBMP_T2T( bmp_t, ptr, rev_bmp );	
		*alp_flag=1;		
		break;
	}
...
BOOL BMP_LoadBMP_X2F(TCHAR *fname, BMP_F *bmp_f, POINT *pos, POINT *size, TCHAR *pack)
{
	BITMAPFILEHEADER    *BMPFileHead;
    BITMAPINFOHEADER    *BMPFileInfo;
	...
	if(!_tcsicmp(STD_GetAttr(fname), _T("BJR")))
	{
		ptr += palnum;
	...
	{	
		...
		case 8:		
			ret=BMP_MakeBMP_B2F( bmp_f, ptr, rev_bmp, palnum );	
			break;

		case 24:	
			ptr += palnum;	
			ret=BMP_MakeBMP_F2F( bmp_f, ptr, rev_bmp );			
			break;	
		}
	}
	GlobalFree( buf );
...
BOOL BMP_LoadBMP_X2H( TCHAR *fname, BMP_H *bmp_h, POINT *pos, POINT *size, int *alp_flag, TCHAR *pack )
{
	BITMAPFILEHEADER    *BMPFileHead;
    BITMAPINFOHEADER    *BMPFileInfo;
	char				*buf, *ptr;
	...
	*alp_flag = BMPFileHead->bfReserved1;
	...
	case 8:		
		ret=BMP_MakeBMP_B2H( bmp_h, ptr, rev_bmp, *alp_flag, palnum );	
		break;

	case 24:	
		ptr += palnum;	
		ret=BMP_MakeBMP_F2H( bmp_h, ptr, rev_bmp );						
		break;
	
	case 32:	
		ptr += palnum;	
		ret=BMP_MakeBMP_T2H( bmp_h, ptr, rev_bmp );	
		*alp_flag=1;		
		break;
	}
...
BOOL BMP_LoadBMP_X2B( TCHAR *fname, BMP_BT *bmp_bt, POINT *pos, POINT *size, int *alp_flag, TCHAR *pack )
{
	BITMAPFILEHEADER    *BMPFileHead;
    BITMAPINFOHEADER    *BMPFileInfo;
	char				*buf, *ptr;
	...
	case 8:		
		ret=BMP_MakeBMP_B2B( bmp_bt, ptr, rev_bmp, *alp_flag, palnum );	
		break;
	}
	GlobalFree( buf );
	return ret;
...
BOOL BMP_CheckBMPBpp( TCHAR *fname )
{
	BITMAPFILEHEADER    *BMPFileHead;
    BITMAPINFOHEADER    *BMPFileInfo;
	char				*buf, *ptr;
	int					ret=0;
	if( !PAC_LoadFile( PackBmpDir, fname, &buf ) ) 
		return ret;
	ptr = buf;
	BMPFileHead = (BITMAPFILEHEADER*)ptr;	
	ptr += sizeof( BITMAPFILEHEADER);	
...
BOOL BMP_CheckBMPBpp2( TCHAR *fname )
{
	BITMAPFILEHEADER    *BMPFileHead;
    BITMAPINFOHEADER    *BMPFileInfo;
	char				*buf, *ptr;
...
BOOL BMP_LoadBMP_T2T( TCHAR *fname, BMP_T *bmp_t )
{
	int					y,w,h;
	BITMAPFILEHEADER    *BMPFileHead;
    BITMAPINFOHEADER    *BMPFileInfo;
	...
	{
		GlobalFree(buf);
		DebugBox( NULL, _T("BMPデータが３２ビットではありません[%s]"), fname );
		return FALSE;
	}
	if( 0>BMPFileInfo->biHeight )
	{	
...
BOOL BMP_LoadBMP_F2T( TCHAR *fname, BMP_T *bmp_t )
{
	int					x,y,w,h;
	BITMAPFILEHEADER    *BMPFileHead;
    BITMAPINFOHEADER    *BMPFileInfo;
	...
	{
		GlobalFree(buf);
		DebugBox( NULL, _T("BMPデータが２４ビットではありません[%s]"), fname );
		return FALSE;
	}	
	if( 0>BMPFileInfo->biHeight )
	{	
...
BOOL BMP_LoadBMP_B2T( TCHAR *fname, BMP_T *bmp_t )
{
	int					x,y,w,h;
	BITMAPFILEHEADER    *BMPFileHead;
    BITMAPINFOHEADER    *BMPFileInfo;
...
BOOL BMP_LoadBMP_F2F( TCHAR *fname, BMP_F *bmp_f )
{
	BITMAPFILEHEADER    *BMPFileHead;
    BITMAPINFOHEADER    *BMPFileInfo;
	char				*buf, *ptr;
...
BOOL BMP_LoadBMP_B2F( TCHAR *fname, BMP_F *bmp_f )
{
	BITMAPFILEHEADER    *BMPFileHead;
    BITMAPINFOHEADER    *BMPFileInfo;
	int					palnum;
...
BOOL BMP_LoadBMP_T2H( TCHAR *fname, BMP_H *bmp_h )
{
	int					x,y,w,h;
	BITMAPFILEHEADER    *BMPFileHead;
    BITMAPINFOHEADER    *BMPFileInfo;
...
BOOL BMP_LoadBMP_F2H( TCHAR *fname, BMP_H *bmp_h )
{
	int					x,y,w,h;
	BITMAPFILEHEADER    *BMPFileHead;
    BITMAPINFOHEADER    *BMPFileInfo;
...
BOOL BMP_LoadBMP_H2H( TCHAR *fname, BMP_H *bmp_h )
{
	int					x,y,w,h;
	BITMAPFILEHEADER    *BMPFileHead;
    BITMAPINFOHEADER    *BMPFileInfo;
	...
	{
		GlobalFree(buf);
		DebugBox( NULL, _T("BMPデータが１６ビットではありません[%s]"), fname );
		return FALSE;
	}
	if( 0>BMPFileInfo->biHeight )
	{	
...
BOOL BMP_LoadBMP_B2H( TCHAR *fname, BMP_H *bmp_h )
{
	int					x,y,w,h,i;
	BITMAPFILEHEADER    *BMPFileHead;
    BITMAPINFOHEADER    *BMPFileInfo;
...
BOOL BMP_LoadBMP_B2B( TCHAR *fname, BMP_BT *bmp_bt  )
{
	return BMP_LoadBMP_B2B_Key( fname, bmp_bt, NULL );
}

BOOL BMP_LoadBMP_B2B_Key( TCHAR *fname, BMP_BT *bmp_bt, int *key )
{
	BITMAPFILEHEADER    *BMPFileHead;
    BITMAPINFOHEADER    *BMPFileInfo;
    RGB32             *pal;
	...
	{
		GlobalFree(buf);
		DebugBox( NULL, _T("BMPデータが８ビットではありません[%s]"), fname);
		return FALSE;
	}
	if( BMPFileHead->bfReserved1 )
	{
	...
	if( 0>BMPFileInfo->biHeight )
	{	
		BMPFileInfo->biHeight*=-1;	rev_bmp=0;	
	}
...
{
	if( bmp_t->buf ) 
		bmp_t->buf = (RGB32 *)GlobalFree( bmp_t->buf );	
	bmp_t->sx = 0;
	bmp_t->sy = 0;
}
...
{
	if( bmp_f->buf ) 
		bmp_f->buf = (RGB24 *)GlobalFree( bmp_f->buf );	
	bmp_f->sx = 0;
	bmp_f->sy = 0;
}
...
{
	if( bmp_h->buf ) 
		bmp_h->buf = (WORD *)GlobalFree( bmp_h->buf );	
	if( bmp_h->alp ) 
		bmp_h->alp = (char *)GlobalFree( bmp_h->alp );	
	bmp_h->sx = 0;
	bmp_h->sy = 0;
}
...
int BMP_SaveBmp_T( TCHAR *fname, BMP_T *bmp_t )
{
	DWORD               actualRead;
	HANDLE              hfile;
	BITMAPFILEHEADER    BMPFileHead;
	...
	if( !WriteFile( hfile, &BMPFileHead, sizeof(BITMAPFILEHEADER), &actualRead, NULL)) 
		return FALSE;
	if( !WriteFile( hfile, &BMPFileInfo, sizeof(BITMAPINFOHEADER), &actualRead, NULL)) 
		return FALSE;
    if( !WriteFile( hfile, bmp_t->buf, BmpAmount*4, &actualRead, NULL) ) 
		return FALSE;
	CloseHandle(hfile);
	return TRUE;
}

int BMP_SaveBmp_F( TCHAR *fname, BMP_F *bmp_f )
{
	DWORD               actualRead;
	HANDLE              hfile;
	BITMAPFILEHEADER    BMPFileHead;
...
int BMP_SaveBmp_B( TCHAR *fname, BMP_BT *bmp_bt )
{
	DWORD               actualRead;
	HANDLE              hfile;
	BITMAPFILEHEADER    BMPFileHead;
	...
	if( !WriteFile( hfile, &BMPFileInfo, sizeof(BITMAPINFOHEADER), &actualRead, NULL)) 
		return FALSE;
	if( !WriteFile( hfile, bmp_bt->pal, 256*sizeof(RGB32), &actualRead, NULL)) 
		return FALSE;
    if( !WriteFile( hfile, bmp_bt->buf, BmpAmount, &actualRead, NULL) ) 
		return FALSE;
	CloseHandle(hfile);
	return TRUE;
}

int BMP_MakeAlphaFF2T( TCHAR *bfname, TCHAR *afname, TCHAR *tfname, int alp_flag, int alp_end, int alp250, int CutSave )
{
	...
	if( !ReadFile( hfile, &BMPFileHead, sizeof( BITMAPFILEHEADER), &actualRead, NULL)) 
		return 2;
	if( !ReadFile( hfile, &BMPFileInfo, sizeof( BITMAPINFOHEADER), &actualRead, NULL)) 
		return 2;
	if( BMPFileInfo.biBitCount != 24)
	{
		CloseHandle(hfile);
    ...
    if( !ReadFile( hfile, buf, sizeRGB, &actualRead, NULL) ) 
		return 2;
	CloseHandle(hfile);
	i=0;j=0;
	if(alp250)
	...
	if(!CutSave)
	{
		...
		if( !WriteFile( hfile, &BMPFileInfo, sizeof( BITMAPINFOHEADER), &actualRead, NULL)) 
			return 11;
		if( !WriteFile( hfile, bmp_t.buf, sizeARGB, &actualRead, NULL) ) 
			return 11;
		CloseHandle(hfile);
...
int BMP_MakeAlphaBB2BA( TCHAR *bfname, TCHAR *afname, TCHAR *tfname, int alp250, int CutSave )
{
    BITMAPFILEHEADER    BMPFileHead;
    BITMAPINFOHEADER    BMPFileInfo;
    RGB32               pal[256];
	...
	if( !ReadFile( hfile, pal, sizeof(RGB32)*palnum, &actualRead, NULL)) 
		return 6;
	sizeBYTE = (BMPFileInfo.biWidth  + revBYTE) * BMPFileInfo.biHeight;
	bmp_bt.buf = (char *)GlobalAlloc( GPTR, sizeof(char)*sizeBYTE );
	buf        = (char *)GlobalAlloc( GPTR, sizeof(char)*sizeBYTE );
	if( !ReadFile( hfile, buf, sizeBYTE, &actualRead, NULL) ) 
		return 6;
	CloseHandle(hfile);
	i=0;j=0;
	if(alp250)
	...
	if(!CutSave)
	{
		...
		if( !WriteFile( hfile, &BMPFileInfo, sizeof(BITMAPINFOHEADER), &actualRead, NULL)) 
			return 11;
		if( !WriteFile( hfile, pal, sizeof(RGB32)*palnum, &actualRead, NULL) ) 
			return 11;
	    if( !WriteFile( hfile, bmp_bt.buf, sizeBYTE, &actualRead, NULL) ) 
			return 11;
	    if( !WriteFile( hfile, bmp_bt.alp, sizeBYTE, &actualRead, NULL) ) 
			return 11;
	}
	else
	{
...
RGB32 BMP_ChangeBright_Full( RGB32 src, int r, int g, int b )
{
	...
	if( rr<=0x80 )	
		...
		wr = (((0xff-wr)*(rr-0x7f))>>7)+wr;
	if( gg<=0x80 )	
		wg =  (      wg * gg      )>>7;
	else			
		wg = (((0xff-wg)*(gg-0x7f))>>7)+wg;
	if( bb<=0x80 )	
		wb =  (      wb * bb      )>>7;
	else			
		wb = (((0xff-wb)*(bb-0x7f))>>7)+wb;
	src.r   = (char)wr;
	src.g = (char)wg;
	src.b  = (char)wb;
	return src;
...
WORD BMP_ChangeBright_High( WORD src, int r, int g, int b )
{
	...
	rb = rgb16.mask_b - wb;
	if( r<=0x80 )
		wr = ( ( wr *  r         )>>7    )&rgb16.mask_r;
	else			
		wr = ((( rr * (r-0x80))>>7)+wr)&rgb16.mask_r;
	if( g<=0x80 )
		wg = ( ( wg *  g         )>>7    )&rgb16.mask_g;
	else			
		wg = ((( rg * (g-0x80))>>7)+wg)&rgb16.mask_g;
	if( b<=0x80 )
		wb = ( ( wb *  b         )>>7    )&rgb16.mask_b;
	else			
		wb = ((( rb * (b-0x80))>>7)+wb)&rgb16.mask_b;
...
WORD BMP_ChangeBright_TH( RGB32 src, int r, int g, int b )
{
	...
	wb = src.b;
	if( r<=0x80 )
		wr = (( (      wr * r         )>>7    )<<sr)&rgb16.mask_r;
	else			
		wr = (((((0xff-wr)*(r-0x80))>>7)+wr)<<sr)&rgb16.mask_r;
	if( g<=0x80 )
		wg = (( (      wg * g         )>>7    )<<sg)&rgb16.mask_g;
	else			
		wg = (((((0xff-wg)*(g-0x80))>>7)+wg)<<sg)&rgb16.mask_g;
	if( b<=0x80 )
		wb = (( (      wb * b         )>>7    )>>sb)&rgb16.mask_b;
	else			
		wb = (((((0xff-wb)*(b-0x80))>>7)+wb)>>sb)&rgb16.mask_b;
	return (WORD)(wr|wg|wb);
}
...
static void BMP_CreateTableBright_T( void )
{
	...
	for( i=0 ; i<512 ; i++ )
		AddTable[i] = min(i,255);	
	for( i=0 ; i<512 ; i++ )
		SubTable[i] = max(i-255,0);	
	for( i=0 ; i<256 ; i++ )
	{
		for( j=0 ; j<256 ; j++ )
		{
...
static void CreateHighColorBrightTable( int r,int g, int b, WORD *high_tbl )
{
	...
	{ 
		for(i=0; i<db ;i++)	
			btbl[i] = (((rgb16.dep_b-i)*b2)>>7)+i;					
	}
	if(g<=0x80) 
	{ 
		for(i=0; i<dg ;i++)	
			gtbl[i] = ((i*g)>>7)<<rgb16.pos_g;						
	}
	else		
	{ 
		for(i=0; i<dg ;i++)	
			gtbl[i] = ((((rgb16.dep_g-i)*g2)>>7)+i)<<rgb16.pos_g;	
	}
	if(r<=0x80)
	{
		for( rr=0 ; rr < dr ; rr++ )
...
int BMP_EncodeVisual( char *dst_fname, char *src_fname, char *dir )
{
	...
	TCHAR				src_path[1024];
	TCHAR				dst_path[1024];
	wsprintf( src_path, _T("%s\\%s"), dir, src_fname );
	wsprintf( dst_path, _T("%s\\%s"), dir, dst_fname );
    ...
    if(palnum)
	{
		if( !ReadFile( hfile, buf, palnum, &actualRead, NULL) ) 
		{ 
			CloseHandle(hfile); 
			return 2; 
		}
	}
    if( !ReadFile( hfile, buf, size, &actualRead, NULL) ) 
	{ 
		CloseHandle(hfile); 
		return 2; 
	}
	...
	for(j=0; j<h ;j++ )
	{
		...
		for(x=0; x<BMPFileInfo.biWidth*3 ;x++ )
		{
			...
			if(rev)	
				k = x+(h-1-j)*BMPFileInfo.biWidth*3;
			else	
				k = x+j*BMPFileInfo.biWidth*3;
			if(x%2)
			{
	...
	if( !WriteFile( hfile, buf2, size, &actualRead, NULL) ) 
		return 12;
	CloseHandle(hfile);
	GlobalFree( buf );
	return 0;
