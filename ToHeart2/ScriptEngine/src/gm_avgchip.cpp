...
void AVG_PlayChipBack( void )
{
	...
	ChipDrawPose = OFF;	
	for( i=0 ; i<GRP_BACK_CHIP ; i++ )	
		DSP_SetGraphDisp( GRP_BACK+i, ON );
	for( i=0; i<GRP_CHAR_CHIP ;i++ )
	{
...
{
	int		i;
	for(i=GRP_BACK2;i<GRP_CHAR;i++) 
		DSP_ResetGraph( i );
	for(i=BMP_BACK;i<BMP_CHAR;i++) 
		DSP_ReleaseBmp( i );
	for(i=GRP_WORK;i<GRP_SCRIPT;i++) 
		DSP_ResetGraph( i );
	if(bmp_init) 
		for(i=GRP_SCRIPT;i<GRP_ENDING;i++) 
			DSP_ResetGraph( i );
	for(i=GRP_SYSTEM2;i<GRP_SELECT;i++) 
		DSP_ResetGraph( i );
	DSP_ResetGraph( GRP_SPBACK );
	DSP_ReleaseSpriteAll();
	for(i=0;i<GRP_BACK_CHIP+GRP_CHAR_CHIP;i++) 
		DSP_ReleaseSprite( i );
		...
		else
		{
			...
			case BAK_CFADE:
				if(sp)	
					...
					DSP_SetGraphParam( GRP_WORK, DRW_AMI(rate) );
				break;

			case BAK_CFADE_UP:
				if(sp)	
					DSP_SetGraphParam( GRP_WORK, DRW_LCF(UP,rate) );
				else	
					DSP_SetGraphParam( GRP_WORK, DRW_LPP(UP,rate) );
				break;

			case BAK_CFADE_DO:
				if(sp)	
					DSP_SetGraphParam( GRP_WORK, DRW_LCF(DO,rate) );
				else	
					DSP_SetGraphParam( GRP_WORK, DRW_LPP(DO,rate) );
				break;

			case BAK_CFADE_RI:
				if(sp)	
					DSP_SetGraphParam( GRP_WORK, DRW_LCF(RI,rate) );
				else	
					DSP_SetGraphParam( GRP_WORK, DRW_LPP(RI,rate) );
				break;

			case BAK_CFADE_LE:
				if(sp)	
					DSP_SetGraphParam( GRP_WORK, DRW_LCF(LE,rate) );
				else	
					DSP_SetGraphParam( GRP_WORK, DRW_LPP(LE,rate) );
				break;

			case BAK_CFADE_CE:
				if(sp)	
					DSP_SetGraphParam( GRP_WORK, DRW_LCF(CE,rate) );
				else	
					DSP_SetGraphParam( GRP_WORK, DRW_LPP(CE,rate) );
				break;

			case BAK_CFADE_OU:
				if(sp)	
					DSP_SetGraphParam( GRP_WORK, DRW_LCF(OU,rate) );
				else	
					DSP_SetGraphParam( GRP_WORK, DRW_LPP(OU,rate) );
				break;

			case BAK_DIA1:	case BAK_DIA2:	case BAK_DIA3:
				if(sp)	
					DSP_SetGraphParam( GRP_WORK, DRW_DIO(BackStruct.fd_type-BAK_DIA1,rate) );
				else	
					DSP_SetGraphParam( GRP_WORK, DRW_DIA(BackStruct.fd_type-BAK_DIA1,rate) );
				break;
			...
			case BAK_CFZOOM1:
				...
				if(sp && MainWindow.draw_mode2!=16)	
					...
					DSP_SetGraphParam( GRP_WORK, DRW_AMI(256-rate)   );
...
void AVG_SetChipBack( int bak_no, int wx, int wy, int ww, int wh )
{
	...
	if(BackStruct.tone_back==-1)
		wsprintf( tone_fname2, "tb%02d%1d.amp", bak_no, BackStruct.tone_no );
	else						
		wsprintf( tone_fname2, "tb%02d%1d.amp", bak_no, BackStruct.tone_back );
...
BOOL AVG_SetChipBackParts( int bak_ch, int dx, int dy, int w, int h, int lw, int ln, int depth, int nuki, int lno, int cfade, DWORD param )
{
	...
	if( lno<4 )	
		...
		layer = LAY_FORE+lno%4;
	if(ln)
	{
		if(lw)	
			gnum = min( (DISP_X+lw-1)/lw+1, ln );
		else	
			gnum = 1;
	}
	else
	...
	{	
		...
		case SCRLL_BRAKE:
			if(ChipBackStruct.sc_cnt<sc_max)
			{
				...
				if(ChipBackStruct.scroll==SCRLL_ACCEL) 
					count = count*count/sc_max;
				if(ChipBackStruct.scroll==SCRLL_BRAKE) 
					count = sc_max-(sc_max-count)*(sc_max-count)/sc_max;
...
static BOOL LoadChipChar( int char_no, int pose, int end, int lnum )
{
	...
	else
	{
		...
		if(BackStruct.tone_no==TONE_SEPIA) 
			spia=ON;
	}
...
void AVG_SetChipCharRev( int char_no, int rev )
{
	...
	if( ChipChar[ch_no].flag )
	{
		...
		if(rev)	
			DSP_SetGraphRevParam( ChipChar[ch_no].gno, REV_W );
		else	
			DSP_SetGraphRevParam( ChipChar[ch_no].gno, REV_NOT );
	}
}
		...
		if(!ChipDrawPose)
		{
			for( i=0; i<GRP_CHAR_CHIP ;i++ )
			{
				if( ChipChar[i].flag )
				{
					...
					{
						int	r,g,b;
						if( BackStruct.rr<128 )	
							r = ChipChar[i].r*BackStruct.rr/128;
						else					
							r = (0xff-ChipChar[i].r)*(BackStruct.rr-128) / 128 + ChipChar[i].r;
						if( BackStruct.gg<128 )	
							g = ChipChar[i].g*BackStruct.gg/128;
						else					
							g = (0xff-ChipChar[i].g)*(BackStruct.gg-128) / 128 + ChipChar[i].g;
						if( BackStruct.bb<128 )	
							b = ChipChar[i].b*BackStruct.bb/128;
						else					
							b = (0xff-ChipChar[i].b)*(BackStruct.bb-128) / 128 + ChipChar[i].b;
					...
					if( se && ChipChar[i].smax )
					{
						...
						default: 
							se = ChipChar[i].sound[se][ChipChar[i].scnt]; 
							break;	
						}
