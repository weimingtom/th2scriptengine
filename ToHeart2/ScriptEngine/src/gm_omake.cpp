	...
	for(i=0;i<3*4;i++)
	{
		...
		if(CgList2[page_no*12+i][0])
		{
			if(cgno)
			{
				for(j=0;j<10;j++)
				{
					if(!CgList[page_no*12+i][j])
						break;
				}
				SamAmount[i] = j;
				if(cgno>=100000)
					wsprintf( buf, "th0%04d%d.tga", (cgno-100000)/10, CgList2[page_no*12+i][0]%10 );
				else			
					wsprintf( buf, "tv0%04d%d.tga", cgno/10, CgList2[page_no*12+i][0]%10 );
...
int OMK_ControlCgMode( void )
{
	...
	if(GameKey.r) 
	{
		if(select<12)
		{
			select = (select+1>=3*4)? select: select+1;	
			key_flag = 1;
		}
		else if(select ==12 )
		{
	...
	if(GameKey.d) 
	{
		...
		{
			select = (select+4>=3*4)? select: select+4; 
			key_flag = 2;
		}
	}
	if(key_flag)
	...
	case CG_VIEW_OPEN:
		...
		if(count==0)
		{
			...
			char	buf[256];
			if(cgno>=100000)	
				wsprintf( buf, "h0%05d.tga", cgno%100000 );
			else				
...
void LoadReplayWindow( void )
{
	...
	for(i=0;i<9;i++)
	{
		...
		case 0:	case 1:	case 2:	case 3: case 4:	
			check = AVG_GetReplayFlag(i+1);	
			break;

		case 5:	case 6:	case 7:	
			check = AVG_GetReplayFlag(i+2);	
			break;

		case 8:	
			check = AVG_GetReplayFlag(i+3);	
			break;
		}
		if(check && EroTbl2[i])
		{
...
int OMK_ControlReplayMode( void )
{
	...
	if(GameKey.r) 
	{	
		select = (select+1>7)? select: select+1;	
		key_flag=1; 
	}
	if(GameKey.u) 
	{
	...
	if(GameKey.d) 
	{
		...
		{
			select = (select+4>=9)? select: select+4; 
			key_flag=2;
		}
	}
	if(key_flag)
		...
		if(select!=-1)
		{
			...
			if( click )
			{
				...
				default:
					...
					{
					case 0:	case 1:	case 2:	case 3: case 4:	
						check = AVG_GetReplayFlag(select+1);	
						break;
					
					case 5:	case 6:	case 7:	
						check = AVG_GetReplayFlag(select+2);	
						break;
					
					case 8:	
						check = AVG_GetReplayFlag(select+3);	
						break;
					}
					if( check )
					{
...
int OMK_ControlSoundMode( void )
{
	...
	if(GameKey.d) 
	{	
		select = (select+1>=40)? select: select+1;	
		key_flag=1; 
	}
	if(GameKey.l) 
	{
		if(select<40)
		{
			select = (select-10< 0)? select: select-10; 
			key_flag=2;
		}
		else if(select==40)
		{
	...
	if(GameKey.r) 
	{
		select = (select+10>=40)? 40 : select+10; 
		key_flag=2;
	}
	if(key_flag)
	{
			...
			if( click )
			{
				...
				default:
					...
					else
					{
						...
						{
						default:
							AVG_SizukuPlayVoice( 0, 6900+select, -1, -1, 256, FALSE,TRUE );	
							break;
						
						case 6:	
							AVG_SizukuPlayVoice( 0, 6900+7, -1, -1, 256, FALSE,TRUE );	
							break;
						
						case 7:	
							AVG_SizukuPlayVoice( 0, 6900+6, -1, -1, 256, FALSE,TRUE );	
							break;
						}
						DSP_SetGraphDisp( CGGP_SYS+1, ON );
						DSP_SetGraphPos( CGGP_SYS+1, 532, 107+sel_mus*35, 532, 107+sel_mus*35, 184, 35 );
