...
BOOL DRW_DrawBMP_HHH( DRAW_OBJECT dobj )
{
	...
	if( src1->buf==NULL ) 
	{ 
		DebugPrintf(_T("画像領域1がありません\n")); 
		return FALSE; 
	}
	if( src2->buf==NULL ) 
	{ 
		DebugPrintf(_T("画像領域2がありません\n")); 
		return FALSE; 
	}
	if( dobj.clip!=(void*)-1 )
	{
...
BOOL DRW_DrawBMP_HH( DRAW_OBJECT dobj )
{
	...
	default:
	case DRW_NML:	
		DRW_DrawBMP_HH_Std( &dobj );	
		break;	

	case DRW_BLD2:	
		DRW_DrawBMP_HH_Bld( &dobj );	
		break;	

	case DRW_ADD:	
		DRW_DrawBMP_HH_Add( &dobj );	
		break;	

	case DRW_SUB:	
		DRW_DrawBMP_HH_Sub( &dobj );	
		break;	

	case DRW_MUL:	
		DRW_DrawBMP_HH_Mul( &dobj );	
		break;	

	case DRW_AMI2:	
		DRW_DrawBMP_HH_Ami( &dobj );	
		break;	

	case DRW_NEG:	
		DRW_DrawBMP_HH_Neg(	&dobj );	
		break;	

	case DRW_SUI2:	
		DRW_DrawBMP_HH_Sui(	&dobj );	
		break;	

	case DRW_SBL2:	
		DRW_DrawBMP_HH_Sbl(	&dobj );	
		break;	

	case DRW_LST2:	
		DRW_DrawBMP_HH_Lst(	&dobj );	
		break;	
	
	case DRW_LBL2:	
		DRW_DrawBMP_HH_Lbl(	&dobj );	
		break;	
...
BOOL DRW_DrawBMP_HB( DRAW_OBJECT dobj )
{
	...
	else
	{
		...
		{						
			...
			{
			case DRW_SBL2:	
				BMP_ChangePalBright_High( pal, 128/2, 128/2, 128/2 );	
				break;

			case DRW_BLD2:	
				BMP_ChangePalBright_High( pal, dobj.dnum/2, dobj.dnum/2, dobj.dnum/2 );	
				break;

			case DRW_NEG:	
				BMP_ChangePalNega_High( pal );		
				break;	
			}
		}
	}
	...
	case DRW_VIV2:
	case DRW_NML:	
		DRW_DrawBMP_HB_Std( &dobj, &src2 );	
		break;	

	case DRW_BLD2:	
		DRW_DrawBMP_HB_Bld( &dobj, &src2 );	
		break;	

	case DRW_ADD:	
		DRW_DrawBMP_HB_Add( &dobj, &src2 );	
		break;	

	case DRW_SUB:	
		DRW_DrawBMP_HB_Sub( &dobj, &src2 );	
		break;	

	case DRW_MUL:	
		DRW_DrawBMP_HB_Mul( &dobj, &src2 );	
		break;	

	case DRW_AMI2:	
		DRW_DrawBMP_HB_Ami( &dobj, &src2 );	
		break;	

	case DRW_SUI2:	
		DRW_DrawBMP_HB_Sui(	&dobj, &src2 );	
		break;	

	case DRW_SBL2:	
		DRW_DrawBMP_HB_Sbl(	&dobj, &src2 );	
		break;	
	}
	return ret;
}
...
BOOL DRW_DrawZOOM_HH_Bld( DRAW_OBJECT *dobj, int dw2, int dh2, int sw2, int sh2, int xxx, int yyy )
{
	...
	if( blnd==256 )
	{ 
		return DRW_DrawZOOM_HH_Std( dobj, dw2, dh2, sw2, sh2, xxx, yyy );	
	}	
	px = sw2/dw2;
	wx = sw2%dw2;
	py = sh2/dh2 * src->sx;
...
BOOL DRW_DrawZOOM_HH( DRAW_OBJECT dobj )
{
	...
	default:
	case DRW_NML:	
		DRW_DrawZOOM_HH_Std( &dobj, dw2, dh2, sw2, sh2, xx, yy );	
		break;

	case DRW_BLD2:	
		DRW_DrawZOOM_HH_Bld( &dobj, dw2, dh2, sw2, sh2, xx, yy );	
		break;	

	case DRW_AMI2:	
		DRW_DrawZOOM_HH_Ami( &dobj, dw2, dh2, sw2, sh2, xx, yy );	
		break;	
	}
	return TRUE;
}
...
BOOL DRW_DrawZOOM_HB( DRAW_OBJECT dobj )
{
	...
	case DRW_VIV2:	
	case DRW_NML:	
		DRW_DrawZOOM_HB_Std( &dobj, dw2, dh2, sw2, sh2, xx, yy, &src2 );	
		break;

	case DRW_BLD2:	
		DRW_DrawZOOM_HB_Bld( &dobj, dw2, dh2, sw2, sh2, xx, yy, &src2 );	
		break;

	case DRW_ADD:	
		DRW_DrawZOOM_HB_Add( &dobj, dw2, dh2, sw2, sh2, xx, yy, &src2 );	
		break;	
	}
	return TRUE;
}
					   ...
					   int r, int g, int b, int param, int blnd )
{
	...
	default:
	case DRW_NML:	
		DRW_DrawXLine_HH_Std( dest_p, src, sx2, sy2, sx/ww, sy/ww, dsx, dsy, fsx, fsy, w, ww, nuki, r, g, b );				
		break;	
...
BOOL DRW_DrawXLine_HB( WORD *dest_p,int dy, BMP_BH *src, MIN_MAX *mi, MIN_MAX *ma, int nuki, RECT *clip, int param, int blnd )
{
	...
	switch( param )
	{
		...
		case DRW_VIV2:	
		case DRW_NML:	
			DRW_DrawXLine_HB_Std( dest_p, src, sx2, sy2, sx/ww, sy/ww, dsx, dsy, fsx, fsy, w, ww, nuki );				
			break;	
	}
	return ret;
}
