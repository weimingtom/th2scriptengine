...
{
	int  i;
	for(i=RBUF_SIZE+1; i<=RBUF_SIZE+256; i++) 
		rson[i]	= NIL;
...
int	InitEncode(TCHAR *fname, char **dst, char **src)
{
	int	size;
	if( fname == NULL ) 
		return FALSE;
	...
	if(size)
	{
		*dst = (char *)GAlloc( size+8 );
	}
	return size;
}

int	 LZS_EncodeMemory( char *dst, char *src, int file_size )
{
	...
	code_buf_ptr = mask = 1;
	s = 0;  
	r = RBUF_SIZE - UPPER_LIMIT;
	FillMemory(&text_buf[0], r, ' ');
	for( len=0 ; (len<UPPER_LIMIT)&&(src_pos!=file_size) ; len++, src_pos++) 
		text_buf[r+len] = src[src_pos];
	if( len == 0 ) 
		return FALSE;
	for( i=1 ; i<=UPPER_LIMIT ; i++ ) 
		InsertNode(r-i);
...
int	 LZS_EncodeMemoryBmpTbl( char *dst, char *src, DWORD *tbl, int file_size, int key )
{
	int		i, len, r, s, last_match_length, code_buf_ptr;
	char	code_buf[17], mask;
	int		dst_pos=0, src_pos=0;
	...
	do {
		...
		for( i=0 ; (i<last_match_length)&&(src_pos<file_size) ; i++,src_pos++ )
		{
			...
			text_buf[s] = src[src_pos];
			if( s < UPPER_LIMIT-1 ) 
				text_buf[s+RBUF_SIZE] = src[src_pos];
			s = (s+1) & (RBUF_SIZE-1);  
			r = (r+1) & (RBUF_SIZE-1);
			InsertNode(r);
		}
...
int LZS_Encode(TCHAR *src_name, TCHAR *dst_name)
{
...
static int	InitDecode(TCHAR *filename, char **dst, char **src, int *decode_size )
{
	DWORD	dum, type;
	HANDLE	hfile;
	int		file_size;
	...
	*src = (char *)GAlloc( file_size );
	ReadFile( hfile, &type, 4, &dum, NULL );
	ReadFile( hfile, decode_size, sizeof(DWORD), &dum, NULL );
	ReadFile( hfile, *src, file_size, &dum, NULL );
	CloseHandle(hfile);
	*dst = (char *)GAlloc( *decode_size );
	return file_size;
}

int  LZS_DecodeMemory(char *dst, char *src, int file_size, int dst_size)
{
	int		j, c;
	int		ret_size;
	DWORD	flags = 0;
	...
	if(err)
	{
		DebugBox(NULL, _T("エラーだす"));
	}
	return ret_size - (RBUF_SIZE - UPPER_LIMIT);
}

char *LZS_Decode( TCHAR *filename )
{
	char	*src=NULL, *dst=NULL;
...
char *LZS_Decode2(char *src, int file_size)
{
	DWORD	size;
	char	*dst=NULL;	
	size = *(DWORD*)(src+4);
	dst = (char *)GAlloc( size );
