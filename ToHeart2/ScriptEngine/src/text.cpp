...
static int GetDigit( TCHAR *str, int *cnt,int *digit )
{
	int dcnt = 0;
	*digit = 0;
...
{
	TCHAR	*buf = (TCHAR*)GAlloc( strlen(str)+1 );
	TCHAR    *ptr;
	int		i=0,j=0;
...
int TXT_DrawText( void *dest,int draw_mode2, int sx, int sy, int w, int h, int pixh_w, int pich_h, int *px2, int *py2, RECT *clip,
					int font, TCHAR *str, int color, int text_cnt, int step_cnt, int r, int g, int b, int alph, int kage, int kaigyou_musi )
{
					...
					int font, TCHAR *str, int color, int text_cnt, int step_cnt, int r, int g, int b, int alph, int kage, int cnt_flag, TCHAR *normal_str, int kaigyou_musi )
{
	...
	{	
		...
		case '\0':		
			if( tag_cnt )
			{
				DebugBox( NULL, _T("タグが開いたままになっています。必ず閉じてください") );
			}
			end_flag=1;
			break;
		...
		case '<':		
			...
			if( tag_cnt >= 15 )
			{
				DebugBox( NULL, _T("タグを開き過ぎです。ＭＡＸ １５") );
				break;
			}
			switch( str[cnt] )
			{
			...
			case TAG_RUBI:	
				{
					...
					if(!cnt_flag) 
						FNT_DrawText( dest, draw_mode2, rx, py-rfno-1,pw,0, rubi_str, rfno, clip, rgb.r, rgb.g, rgb.b, alph2, (kage)?kage-1:0 );
				}
				cnt--;
				break;
			}
		...
		case '>':		
			if( tag_cnt <= 0 )
			{
				DebugBox( NULL, _T("閉じタグ( > )の数が、開いた数より多くなっています") );
				break;
			}
			tag_cnt--;
			if(!TH2_Flag)
		...
		if(draw_flag)
		{
			...
			{	
				...
				if(!cnt_flag) 
					FNT_DrawChar( dest, draw_mode2, px, py, str[cnt], fno, clip, rgb.r, rgb.g, rgb.b, alph2, kage, 1 );
				cnt2 += TXT_GetMsgSpeed( speed );
				if(accent_flag)
				{
					if(!cnt_flag) 
						FNT_DrawChar( dest, draw_mode2, px+(fno-rfno/2)/2, py-rfno+1, '?', rfno, clip, rgb.r, rgb.g, rgb.b, alph2, (kage)?kage-1:0, 1 );
				}
				px += fno/2+pixh_w;
				kaig=0;
				if(hankaku_err)
				{
					DebugBox( NULL, _T("ToHert2では半角文字列は使用できません！") );
				}
			}
			else
			{		
				if(normal_str) 
				{ 
					*(WORD*)normal_str = *(WORD*)&str[cnt]; 
					normal_str+=2; 
				}
				if(!cnt_flag) 
					FNT_DrawChar( dest, draw_mode2, px, py, *(WORD*)&str[cnt], fno, clip, rgb.r, rgb.g, rgb.b, alph2, kage, 0 );
				cnt++;
				cnt2 += TXT_GetMsgSpeed( speed );
				if(accent_flag)
				{
		...
		if( px-sx >= w*font-fno+1 )
		{
			if( tag_param[tag_cnt-1]!=TAG_RUBI )
			{
				if( kflag==1 )
				{
					if( /*strncmp*/_tcsncmp( &str[ cnt ], _T("　"), 2 ) )
					{
						kflag = 0;
						px_bak = px;
						px = sx;
				...
				{
					if(
						strncmp( &str[ cnt ], _T("。"), 2 ) && 
						strncmp( &str[ cnt ], _T("、"), 2 ) &&
						strncmp( &str[ cnt ], _T("，"), 2 ) && 
						strncmp( &str[ cnt ], _T("．"), 2 ) &&
						strncmp( &str[ cnt ], _T("?"), 2 ) &&   //TODO:
						strncmp( &str[ cnt ], _T("…"), 2 ) &&
						strncmp( &str[ cnt ], _T("ー"), 2 ) && 
						strncmp( &str[ cnt ], _T("ー"), 2 ) &&
						strncmp( &str[ cnt ], _T("："), 2 ) && 
						strncmp( &str[ cnt ], _T("；"), 2 ) &&
						strncmp( &str[ cnt ], _T("？"), 2 ) && 
