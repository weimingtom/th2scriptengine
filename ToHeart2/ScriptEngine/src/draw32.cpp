...
BOOL DRW_DrawBMP_TTT( DRAW_OBJECT dobj )
{
	...
	if( dest->buf==NULL ) 
	{ 
		DebugPrintf(_T("描画領域がありません[DRW_DrawBMP_TTT]\n")); 
		return FALSE; 
	}
	if( src1->buf==NULL ) 
	{ 
		DebugPrintf(_T("画像領域1がありません[DRW_DrawBMP_TTT]\n")); 
		return FALSE; 
	}
	if( src2->buf==NULL ) 
	{ 
		DebugPrintf(_T("画像領域2がありません[DRW_DrawBMP_TTT]\n")); 
		return FALSE; 
	}	
	if( dobj.clip!=(void*)-1 )
	{
	...
	case DRW_BLD2:
		if(dobj2.dest) 
			DRW_DrawBMP_TT_Bld(&dobj2);
		if(dobj3.dest) 
			DRW_DrawBMP_TT_Bld(&dobj3);
		DRW_DrawBMP_TTT_Bld(&dobj);
		break;

	case DRW_AMI2:
		if(dobj2.dest) 
			DRW_DrawBMP_TT_Ami(&dobj2);
		if(dobj3.dest) 
			DRW_DrawBMP_TT_Ami(&dobj3);
	...
	case DRW_LPP2+3:	case DRW_LPP2+4:	case DRW_LPP2+5:	
		if(dobj2.dest) 
			DRW_DrawBMP_TT_Lpp(&dobj2);
		if(dobj3.dest) 
			DRW_DrawBMP_TT_Lpp(&dobj3);
...
BOOL DRW_DrawBMP_TT( DRAW_OBJECT dobj )
{
	...
	if( dest->buf==NULL ) 
	{	
		DebugPrintf(_T("描画領域がありません[DRW_DrawBMP_TT]\n")); 
		return FALSE;
	}
	if( src->buf==NULL  ) 
	{ 
		DebugPrintf(_T("画像領域がありません[DRW_DrawBMP_TT]\n")); 
		return FALSE; 
	}	
	if( dobj.clip!=(void*)-1 )
	{
	...
	default:
	case DRW_NML:	
		DRW_DrawBMP_TT_Std( &dobj );	
		break;	

	case DRW_BLD2:
		DRW_DrawBMP_TT_Bld( &dobj );	
		break;	

	case DRW_ADD:	
		DRW_DrawBMP_TT_Add( &dobj );
		break;	

	case DRW_OOI:	
		DRW_DrawBMP_TT_Ooi( &dobj );
		break;	

	case DRW_SUB:
		DRW_DrawBMP_TT_Sub( &dobj );
		break;	

	case DRW_MUL:	
		DRW_DrawBMP_TT_Mul( &dobj );	
		break;	

	case DRW_AMI2:
		DRW_DrawBMP_TT_Ami( &dobj );
		break;	

	case DRW_NIS2:	
		DRW_DrawBMP_TT_Nis( &dobj );	
		break;	

	case DRW_NEG:	
		DRW_DrawBMP_TT_Neg(	&dobj );
		break;	

	case DRW_VIV2:
		DRW_DrawBMP_TT_Viv( &dobj );	
		break;	

	case DRW_BOM2:	
		DRW_DrawBMP_TT_Bom(	&dobj );
		break;

	case DRW_SUI2:
		DRW_DrawBMP_TT_Sui(	&dobj );
		break;	

	case DRW_SBL2:
		DRW_DrawBMP_TT_Sbl(	&dobj );	
		break;	

	case DRW_WAV2:
		DRW_DrawBMP_TT_Wav(	&dobj );
		break;	

	case DRW_WBL2:
		DRW_DrawBMP_TT_Wbl(	&dobj );
		break;	

	case DRW_LST2:
		DRW_DrawBMP_TT_Lst(	&dobj );
		break;	

	case DRW_LBL2:	
		DRW_DrawBMP_TT_Lbl(	&dobj );
		break;	
...
BOOL DRW_DrawBMP_TB( DRAW_OBJECT dobj )
{
	...
	if( dest->buf==NULL ) 
	{ 
		DebugPrintf(_T("描画領域がありません[DRW_DrawBMP_TB]\n")); 
		return FALSE; 
	}
	if( src->buf==NULL  ) 
	{	
		DebugPrintf(_T("画像領域がありません[DRW_DrawBMP_TB]\n")); 
		return FALSE; 
	}
	if( dobj.clip!=(void*)-1 )
	{
	...
	case DRW_VIV2:
	case DRW_NML:
		DRW_DrawBMP_TB_Std( &dobj, &src2 );	
		break;	

	case DRW_BLD2:	
		DRW_DrawBMP_TB_Bld( &dobj, &src2, src->pal );
		break;	

	case DRW_ADD:	
		DRW_DrawBMP_TB_Add( &dobj, &src2 );
		break;	

	case DRW_OOI:	
		DRW_DrawBMP_TB_Ooi( &dobj, &src2 );	
		break;	

	case DRW_SUB:	
		DRW_DrawBMP_TB_Sub( &dobj, &src2 );	
		break;	

	case DRW_MUL:
		DRW_DrawBMP_TB_Mul( &dobj, &src2 );	
		break;	

	case DRW_AMI2:
		DRW_DrawBMP_TB_Ami( &dobj, &src2 );
		break;	

	case DRW_BOM2:	
		DRW_DrawBMP_TB_Bom( &dobj, &src2 );
		break;	

	case DRW_SUI2:
		DRW_DrawBMP_TB_Sui(	&dobj, &src2 );
		break;	

	case DRW_SBL2:	
		DRW_DrawBMP_TB_Sbl(	&dobj, &src2 );
		break;	
	}
	return ret;
}
...
BOOL DRW_DrawZOOM_TT_Bld( DRAW_OBJECT *dobj, int dw2, int dh2, int sw2, int sh2, int xxx, int yyy )
{
	...
	if( blnd==BLD_0P  )
		return 1;															
	if( blnd==BLD_100P)
		return DRW_DrawZOOM_TT_Std( dobj, dw2, dh2, sw2, sh2, xxx, yyy );	
	px = sw2/dw2 * xinc;
	wx = sw2%dw2;
	py = sh2/dh2 * yinc;
	wy = sh2%dh2;
...
BOOL DRW_DrawZOOM_TT( DRAW_OBJECT dobj )
{
	...
	if( dest->buf==NULL ) 
	{ 
		DebugPrintf(_T("描画領域がありません[DRW_DrawZOOM_TT]\n")); 
		return FALSE; 
	}
	if( src->buf==NULL  ) 
	{	
		DebugPrintf(_T("画像領域がありません[DRW_DrawZOOM_TT]\n")); 
		return FALSE; 
	}
	if( dobj.clip!=(void*)-1 )
	{
	...
	default:
	case DRW_NML:	
		DRW_DrawZOOM_TT_Std( &dobj, dw2, dh2, sw2, sh2, xx, yy );
		break;	

	case DRW_BLD2:	
		DRW_DrawZOOM_TT_Bld( &dobj, dw2, dh2, sw2, sh2, xx, yy );
		break;	

	case DRW_ADD:	
		DRW_DrawZOOM_TT_Add( &dobj, dw2, dh2, sw2, sh2, xx, yy );	
		break;	

	case DRW_SUB:	
		DRW_DrawZOOM_TT_Sub( &dobj, dw2, dh2, sw2, sh2, xx, yy );
		break;	

	case DRW_MUL:	
		DRW_DrawZOOM_TT_Mul( &dobj, dw2, dh2, sw2, sh2, xx, yy );
		break;	

	case DRW_AMI2:	
		DRW_DrawZOOM_TT_Ami( &dobj, dw2, dh2, sw2, sh2, xx, yy );	
		break;	

	case DRW_NEG:	
		DRW_DrawZOOM_TT_Neg( &dobj, dw2, dh2, sw2, sh2, xx, yy );	
		break;	

	case DRW_VIV2:	
		DRW_DrawZOOM_TT_Viv( &dobj, dw2, dh2, sw2, sh2, xx, yy );	
		break;	
	}
	return TRUE;
}
...
BOOL DRW_DrawZOOM_TB_Bld( DRAW_OBJECT *dobj, int dw2, int dh2, int sw2, int sh2, int xxx, int yyy, BMP_BT *src )
{
	...
	if( blnd==BLD_0P  )
		return 1;																	
	if( blnd==BLD_50P )	
		return DRW_DrawZOOM_TB_Bld128( dobj, dw2, dh2, sw2, sh2, xxx, yyy, src );	
	if( blnd==BLD_100P)	
		return DRW_DrawZOOM_TB_Std( dobj, dw2, dh2, sw2, sh2, xxx, yyy, src );		
	px = sw2/dw2 * xinc;
	wx = sw2%dw2;
	py = sh2/dh2 * yinc;
	wy = sh2%dh2;
...
BOOL DRW_DrawZOOM_TB( DRAW_OBJECT dobj )
{
	...
	if( dest->buf==NULL ) 
	{ 
		DebugPrintf(_T("描画領域がありません[DRW_DrawZOOM_TB]\n")); 
		return FALSE; 
	}
	if( src->buf==NULL  ) 
	{	
		DebugPrintf(_T("画像領域がありません[DRW_DrawZOOM_TB]\n")); 
		return FALSE; 
	}
	if( dobj.clip!=(void*)-1 )
	{
	...
	case DRW_VIV2:	
	case DRW_NML:	
		DRW_DrawZOOM_TB_Std( &dobj, dw2, dh2, sw2, sh2, xx, yy, &src2 );	
		break;	

	case DRW_BLD2:
		DRW_DrawZOOM_TB_Bld( &dobj, dw2, dh2, sw2, sh2, xx, yy, &src2 );
		break;	

	case DRW_ADD:
		DRW_DrawZOOM_TB_Add( &dobj, dw2, dh2, sw2, sh2, xx, yy, &src2 );
		break;	

	case DRW_SUB:
		DRW_DrawZOOM_TB_Sub( &dobj, dw2, dh2, sw2, sh2, xx, yy, &src2 );
		break;	

	case DRW_MUL:	
		DRW_DrawZOOM_TB_Mul( &dobj, dw2, dh2, sw2, sh2, xx, yy, &src2 );
		break;	

	case DRW_AMI2:
		DRW_DrawZOOM_TB_Ami( &dobj, dw2, dh2, sw2, sh2, xx, yy, &src2 );
		break;	
	}
	return TRUE;
}
					   ...
					   int r, int g, int b, int param, int blnd )
{
	...
	default:
	case DRW_NML:	
		DRW_DrawXLine_TT_Std( dest_p, src, sx2, sy2, sx/ww, sy/ww, dsx, dsy, fsx, fsy, w, ww, nuki, r, g, b );		
		break;	

	case DRW_BLD2:	
		DRW_DrawXLine_TT_Bld( dest_p, src, sx2, sy2, sx/ww, sy/ww, dsx, dsy, fsx, fsy, w, ww, nuki, r, g, b, blnd );	
		break;	

	case DRW_ADD:	
		DRW_DrawXLine_TT_Add( dest_p, src, sx2, sy2, sx/ww, sy/ww, dsx, dsy, fsx, fsy, w, ww, nuki, r, g, b );		
		break;	

	case DRW_SUB:	
		DRW_DrawXLine_TT_Sub( dest_p, src, sx2, sy2, sx/ww, sy/ww, dsx, dsy, fsx, fsy, w, ww, nuki, r, g, b );			
		break;	

	case DRW_MUL:	
		DRW_DrawXLine_TT_Mul( dest_p, src, sx2, sy2, sx/ww, sy/ww, dsx, dsy, fsx, fsy, w, ww, nuki, r, g, b );		
		break;

	case DRW_AMI2:	
		DRW_DrawXLine_TT_Ami( dest_p, src, sx2, sy2, sx/ww, sy/ww, dsx, dsy, fsx, fsy, w, ww, nuki, r, g, b, blnd,dx1,dy );	
		break;
...
BOOL DRW_DrawXLine_TB( RGB32 *dest_p,int dy, BMP_BT *src, MIN_MAX *mi, MIN_MAX *ma, int nuki, RECT *clip, int param, int blnd )
{
	...
	default:
	case DRW_NML:	
		DRW_DrawXLine_TB_Std( dest_p, src, sx2, sy2, sx/ww, sy/ww, dsx, dsy, fsx, fsy, w, ww, nuki );				
		break;	

	case DRW_BLD2:
		DRW_DrawXLine_TB_Bld( dest_p, src, sx2, sy2, sx/ww, sy/ww, dsx, dsy, fsx, fsy, w, ww, nuki, blnd );		
		break;	

	case DRW_ADD:
		DRW_DrawXLine_TB_Add( dest_p, src, sx2, sy2, sx/ww, sy/ww, dsx, dsy, fsx, fsy, w, ww, nuki );			
		break;	

	case DRW_SUB:	
		DRW_DrawXLine_TB_Sub( dest_p, src, sx2, sy2, sx/ww, sy/ww, dsx, dsy, fsx, fsy, w, ww, nuki );				
		break;	

	case DRW_MUL:	
		DRW_DrawXLine_TB_Mul( dest_p, src, sx2, sy2, sx/ww, sy/ww, dsx, dsy, fsx, fsy, w, ww, nuki );				
		break;	

	case DRW_AMI2:	
		DRW_DrawXLine_TB_Ami( dest_p, src, sx2, sy2, sx/ww, sy/ww, dsx, dsy, fsx, fsy, w, ww, nuki, blnd,dx1,dy );	
		break;	
