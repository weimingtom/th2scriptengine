...
void SPR_DrawParts( BMP_SET *vram, PARTS_STRUCT *ps, CHIP_STRUCT *cs, ANIME_CONTROL *ac, SPRITE_PARAM *sp )
{
	...
	{	
		...
		if(cs->r<=128)	
			...
			r = (255-ps->r)*(cs->r-128)/128+ps->r;
		if(cs->g<=128)	
			g =      ps->g * cs->g     /128;
		else			
			g = (255-ps->g)*(cs->g-128)/128+ps->g;
		if(cs->b<=128)	
			b =      ps->b * cs->b     /128;
		else			
			b = (255-ps->b)*(cs->b-128)/128+ps->b;
		if(sp->r<=128)	
			r =      r * sp->r     /128;
		else			
			r = (255-r)*(sp->r-128)/128+r;
		if(sp->g<=128)	
			g =      g * sp->g     /128;
		else			
			g = (255-g)*(sp->g-128)/128+g;
		if(sp->b<=128)	
			b =      b * sp->b     /128;
		else			
			b = (255-b)*(sp->b-128)/128+b;
		cx = ps->cx;	
		cy = ps->cy;
		dsx = ps->sx;	
		dsy = ps->sy;
		...
		cy = sp->dy + cy;
		{
			...
			if( ac->draw_rect.top > cy )        
				ac->draw_rect.top    = cy;
			if( ac->draw_rect.bottom < cy+dsy ) 
				ac->draw_rect.bottom = cy+dsy;
			if( ac->draw_rect.left > cx )       
				ac->draw_rect.left   = cx;
			if( ac->draw_rect.right < cx+dsx )  
				ac->draw_rect.right  = cx+dsx;
			...
			{	
				...
				{		
					...
					if(bs->bmp_bit==BMP_256P)
					{
						...
						case BMP_FULL:	
							DRW_DrawBMP_FB( dobj, SprInterless );	
							break;
					...
					else
					{
						...
						case BMP_FULL:	
							DRW_DrawBMP_FF( dobj, SprInterless );	
							break;
				...
				else
				{
					...
					case BMP_FULL:
						...
						case BMP_TRUE:	
							DRW_DrawBMP_FT( dobj, SprInterless );	
							break;
						}
						break;
			...
			else
			{
				...
				case BMP_FULL:
					...
					{
					case BMP_256P:	
						DRW_DrawZOOM_FB( dobj, SprInterless );	
						break;

					case BMP_FULL:	
						DRW_DrawZOOM_FF( dobj, SprInterless );	
						break;
					
					case BMP_TRUE:	
						DRW_DrawZOOM_FT( dobj, SprInterless );	
						break;
					}
					break;
				case BMP_TRUE:
					...
					int dx, int dy, int r, int g, int b, RECT *clip, int param, int rparam, int zoom )
{
	...
	if(!ac->flag)
	{ 
		DebugPrintf(_T("AnimeControlに登録されていません[%d]\n"), ani_no); 
		return; 
	}
	if(!AnimeStruct[ac->ano].flag)	
	{ 
		DebugPrintf(_T("AnimeStructに登録されていません[%d]\n"), ac->ano); 
		return; 
	}
...
static int SPR_LoadBmpSet( BMP_SET *bmp_set, TCHAR *fname, TCHAR *tfname, int viv )
{
	BOOL	ret=0;
...
int SPR_LoadBmp( TCHAR *fname, TCHAR *tfname, int viv )
{
	int		i,ret;
	...
	if(i==SPBMP_MAX)
	{
		DebugBox( NULL, _T("スプライトが多過ぎます") );
		return -1;
	}
	ret = SPR_LoadBmpSet( SpBmp+i, fname, tfname,viv );
	if(ret==-1) 
	{
		DebugBox( NULL, _T("スプライトファイルの読み込みに失敗しました[%s]"), fname );
		return -1;
	}
	return i;
}

int SPR_CopyBmp( int src_bno, TCHAR *tfname, int viv, int r, int g, int b )
{
	int	dst_bno,i;
	for( i=0; i<SPBMP_MAX ;i++ )
	{
	...
	dst_bno = i;
	DRAW_OBJECT	dobj;
	if(src_bno<0 || src_bno>SPBMP_MAX) 
	{ 
		DebugPrintf(_T("SPBMP領域エラー[SRC=%d]"), src_bno); 
		return -1; 
	}
	if(dst_bno<0 || dst_bno>SPBMP_MAX) 
	{ 
		DebugPrintf(_T("SPBMP領域エラー[DST=%d]"), dst_bno); 
		return -1; 
	}
...
BOOL SPR_CopySprite( int ani_no1,  int ani_no2, TCHAR *tfname, int viv, int r, int g, int b )
{
	int	i;
...
BOOL SPR_LoadSprite( int ani_no, TCHAR *fname, TCHAR *tfname, int viv )
{
	int				i;
	...
	size = PAC_LoadFile( BMP_GetPackDir(), fname, (char **)&buf);
	if(!size) 
		return FALSE;
	ptr = buf;
	...
	{
		GFree( buf );
		DebugBox(NULL, _T("未知のフォーマットです。[%s]"), fname);
		return FALSE;
	}
	...
	as->ss = (SPRITE_STRUCT *)
		GAlloc(sizeof(SPRITE_STRUCT)*as->ss_max);
	for( i=0 ; i<as->ss_max ; i++ )
	{
		ss = &as->ss[i];
...
BOOL SPR_LoadBmpSprite( int ani_no, TCHAR *fname, int nuki, TCHAR *tfname, int viv )
{
	return TRUE;
}
...
BOOL SPR_SetSprite( int ani_no, int sp_no, int mode, int end, int lnum, int cfade )
{
	...
	if( ac_no==ANIME_CONTROL_MAX )    
	{ 
		DebugPrintf(_T("スプライトの表示数が飽和しました[max=%d]"), ANIME_CONTROL_MAX); 
		return -1; 
	}
	if(ani_no<0 || ANIME_MAX<=ani_no) 
	{ 
		DebugPrintf(_T("存在しないスプライト領域が指定されました[%d]\n"),ani_no);  
		return -1; 
	}
	if(AnimeStruct[ani_no].flag==0)   
	{ 
		DebugPrintf(_T("存在しないスプライト領域が指定されました[%d]\n"), ani_no);  
		return -1; 
	}
	...
	{ 
		DebugPrintf(_T("指定されたスプライトパターンは存在しません[%d]\n"), sp_no);  
		return -1; 
	}
...
{
	if( ac_no<0 || ANIME_CONTROL_MAX<=ac_no )        
	{ 
		DebugPrintf(_T("スプライト制御バッファの指定に誤りがあります[%d]\n"), ac_no); 
		return FALSE; 
	}
	if(AnimeControl[ac_no].flag==0)                  
	{ 
		DebugPrintf(_T("存在しないスプライト制御バッファが指定されました[%d]\n"), ac_no);  
		return FALSE; 
	}
	if(ani_no<0 || ANIME_MAX<=ani_no)                
	{ 
...
BOOL SPR_SetSpriteNo( int ac_no, int sp_no )
{
	if( ac_no<0 || ANIME_CONTROL_MAX<=ac_no )        
	{ 
		DebugPrintf(_T("スプライト制御バッファの指定に誤りがあります1[%d]\n"), ac_no); 
		return FALSE; 
	}
	if(AnimeControl[ac_no].flag==0)                  
	{ 
		DebugPrintf(_T("存在しないスプライト制御バッファが指定されました1[%d]\n"), ac_no);  
		return FALSE; 
	}
	ANIME_CONTROL	*ac = &AnimeControl[ac_no];
	if(sp_no<0 || AnimeStruct[ac->ano].ss_max<=sp_no) 
	{ 
		DebugPrintf(_T("指定されたスプライトパターンは存在しません1[%d]\n"), sp_no);  
		return FALSE; 
	}
	if( ac->sp_no!=sp_no )
	{
...
BOOL SPR_SetSpriteCfade( int ac_no, int flag )
{
	if( ac_no<0 || ANIME_CONTROL_MAX<=ac_no )        
	{ 
		DebugPrintf(_T("スプライト制御バッファの指定に誤りがあります2[%d]\n"), ac_no); 
		return FALSE; 
	}
	if(AnimeControl[ac_no].flag==0)                  
	{ 
		DebugPrintf(_T("存在しないスプライト制御バッファが指定されました2[%d]\n"), ac_no);  
		return FALSE; 
	}
...
BOOL SPR_SetSpriteLoop( int ac_no, int end, int lnum )
{
	if( ac_no<0 || ANIME_CONTROL_MAX<=ac_no )        
	{ 
		DebugPrintf(_T("スプライト制御バッファの指定に誤りがあります3[%d]\n"), ac_no); 
		return FALSE; 
	}
	if(AnimeControl[ac_no].flag==0)                  
	{ 
		DebugPrintf(_T("存在しないスプライト制御バッファが指定されました3[%d]\n"), ac_no); 
		return FALSE; 
	}
...
BOOL SPR_SetSpriteLoopThrough( int ac_no )
{
	if( ac_no<0 || ANIME_CONTROL_MAX<=ac_no )        
	{ 
		DebugPrintf(_T("スプライト制御バッファの指定に誤りがあります4[%d]\n"), ac_no); 
		return FALSE; 
	}
	if(AnimeControl[ac_no].flag==0)                  
	{ 
		DebugPrintf(_T("存在しないスプライト制御バッファが指定されました4[%d]\n"), ac_no);  
		return FALSE; 
	}
...
BOOL SPR_SetSpriteRepeatFind( int ac_no )
{
	if( ac_no<0 || ANIME_CONTROL_MAX<=ac_no )        
	{ 
		DebugPrintf(_T("スプライト制御バッファの指定に誤りがあります5[%d]\n"), ac_no); 
		return FALSE; 
	}
	if(AnimeControl[ac_no].flag==0)                  
	{ 
		DebugPrintf(_T("存在しないスプライト制御バッファが指定されました5[%d]\n"), ac_no);  
		return FALSE; 
	}
...
int SPR_GetSpriteRepeatFind( int ac_no )
{
	if( ac_no<0 || ANIME_CONTROL_MAX<=ac_no )        
	{ 
		DebugPrintf(_T("スプライト制御バッファの指定に誤りがあります6[%d]\n"), ac_no); 
		return REPEAT_NULL; 
	}
	if(AnimeControl[ac_no].flag==0)                  
	{ 
		DebugPrintf(_T("存在しないスプライト制御バッファが指定されました6[%d]\n"),ac_no);  
		return REPEAT_NULL; 
	}
...
RECT *SPR_GetSpriteRect( int ac_no )
{
	if( ac_no<0 || ANIME_CONTROL_MAX<=ac_no )        
	{ 
		DebugPrintf(_T("スプライト制御バッファの指定に誤りがあります7[%d]\n"), ac_no); 
		return NULL; 
	}
	if(AnimeControl[ac_no].flag==0)                  
	{ 
		DebugPrintf(_T("存在しないスプライト制御バッファが指定されました7[%d]\n"),ac_no);  
		return NULL; 
	}
...
BOOL SPR_WaitSpriteCondition( int ac_no )
{
	if( ac_no<0 || ANIME_CONTROL_MAX<=ac_no )        
	{ 
		DebugPrintf(_T("スプライト制御バッファの指定に誤りがあります8[%d]\n"), ac_no); 
		return FALSE; 
	}
	if(AnimeControl[ac_no].flag==0)                  
	{ 
		DebugPrintf(_T("存在しないスプライト制御バッファが指定されました8[%d]\n"), ac_no);  
		return FALSE; 
	}
...
int SPR_GetSpriteMode( int ac_no )
{
	if( ac_no<0 || ANIME_CONTROL_MAX<=ac_no )        
	{ 
		DebugPrintf(_T("スプライト制御バッファの指定に誤りがあります9[%d]\n"), ac_no); 
		return SP_STOP; 
	}
	if(AnimeControl[ac_no].flag==0)                  
	{ 
		DebugPrintf(_T("存在しないスプライト制御バッファが指定されました9[%d]\n"), ac_no);  
		return SP_STOP; 
	}
...
BOOL SPR_SetSpriteMode( int ac_no, int mode )
{
	if( ac_no<0 || ANIME_CONTROL_MAX<=ac_no )        
	{ 
		DebugPrintf(_T("スプライト制御バッファの指定に誤りがあります10[%d]\n"), ac_no); 
		return SP_STOP; 
	}
	if(AnimeControl[ac_no].flag==0)                  
	{ 
		DebugPrintf(_T("存在しないスプライト制御バッファが指定されました10[%d]\n"), ac_no); 
		return SP_STOP; 
	}
...
int SPR_GetSpriteEvent( int ac_no )
{
	if( ac_no<0 || ANIME_CONTROL_MAX<=ac_no )        
	{ 
		DebugPrintf(_T("スプライト制御バッファの指定に誤りがあります11[%d]\n"),ac_no); 
		return 0; 
	}
	if(AnimeControl[ac_no].flag==0)                  
	{ 
		DebugPrintf(_T("存在しないスプライト制御バッファが指定されました11[%d]\n"), ac_no);  
		return 0; 
	}
	int				event=0;
