						...
						int blnd )
{
	...
	if( blnd==BLD_0P )	
		return TRUE;	
	if( blnd==BLD_100P)	
		return PRM_DrawRect_T_Std( dest, dx, dy, w, h, r, g, b );
	if( MMX_Flag )
	{
		for( y=0; y < h ; y++, dest_p+=drev )
		{
					...
					DWORD wparam )
{
	...
	if( !ClipRect2( &dx, &dy, &w, &h, dest->sx, dest->sy, clip ) ) 
		return 1;
	...
	default:
	case DRW_NML:	
		PRM_DrawRect_T_Std( dest, dx, dy, w, h, r, g, b );			
		break;	
	
	case DRW_BLD2:	
		PRM_DrawRect_T_Bld( dest, dx, dy, w, h, r, g, b, blnd );	
		break;	
	
	case DRW_ADD:	
		PRM_DrawRect_T_Add( dest, dx, dy, w, h, r, g, b );			
		break;	
	
	case DRW_SUB:	
		PRM_DrawRect_T_Sub( dest, dx, dy, w, h, r, g, b );			
		break;	
	
	case DRW_MUL:	
		PRM_DrawRect_T_Mul( dest, dx, dy, w, h, r, g, b );			
		break;	
	
	case DRW_AMI2:	
		PRM_DrawRect_T_Ami( dest, dx, dy, w, h, r, g, b, blnd );	
		break;	
...
BOOL PRM_DrawFxLine_T_Bld( RGB32 *dest_p, MIN_MAX *mi, MIN_MAX *ma, RECT *clip, RGB32 *rgb_tbl, int blnd )
{
	...
	if( blnd==BLD_0P )	
		return TRUE;	
	if( blnd==BLD_100P)	
		return PRM_DrawFxLine_T_Std( dest_p, mi, ma, clip, rgb_tbl );	
	if( mi->x == DispWidth )	
		return TRUE;
	dx1	= max( clip->left,  mi->x );
	dx2 = min( clip->right, ma->x );
...
BOOL PRM_DrawGxLine_T( RGB32 *dest_p, MIN_MAX *mi, MIN_MAX *ma, RECT *clip, DWORD wparam )
{
	...
	{
		mir = mi->rgb.r*blnd/256;	
		mig = mi->rgb.g*blnd/256;	
		mib = mi->rgb.b*blnd/256;
		mar = ma->rgb.r*blnd/256;	
		mag = ma->rgb.g*blnd/256;	
		mab = ma->rgb.b*blnd/256;
	}
	else
	{
		mir = mi->rgb.r;	
	...
	default:
	case DRW_NML:	
		ret = PRM_DrawGxLine_T_Std( dest_p, pr,pg,pb, r, g, b, dr, dg, db, fr, fg, fb, w, ww );			
		break;	
	
	case DRW_BLD2:	
		ret = PRM_DrawGxLine_T_Bld( dest_p, pr,pg,pb, r, g, b, dr, dg, db, fr, fg, fb, w, ww, blnd );	
		break;	
	
	case DRW_ADD:	
		ret = PRM_DrawGxLine_T_Add( dest_p, pr,pg,pb, r, g, b, dr, dg, db, fr, fg, fb, w, ww );			
		break;	
	
	case DRW_SUB:	
		ret = PRM_DrawGxLine_T_Sub( dest_p, pr,pg,pb, r, g, b, dr, dg, db, fr, fg, fb, w, ww );			
		break;	
	
	case DRW_MUL:	
		ret = PRM_DrawGxLine_T_Mul( dest_p, pr,pg,pb, r, g, b, dr, dg, db, fr, fg, fb, w, ww );			
		break;	
									...
									DWORD wparam )
{
	...
	if( !ClipRectLine( &x1, &y1, &x2, &y2, dest->sx, dest->sy, clip ) ) 
		return 1;
	if( param==DRW_AMI2 || param== DRW_BLD2 )
	{
		if( blnd==BLD_0P   )
	...
	default:
	case DRW_NML:	
		ret = PRM_DrawFLine_T_Std( dest, x1, y1, x2, y2, r, g, b );			
		break;	

	case DRW_BLD2:	
		ret = PRM_DrawFLine_T_Bld( dest, x1, y1, x2, y2, r, g, b, blnd );	
		break;	

	case DRW_ADD:	
		ret = PRM_DrawFLine_T_Add( dest, x1, y1, x2, y2, r, g, b );			
		break;	

	case DRW_SUB:	
		ret = PRM_DrawFLine_T_Sub( dest, x1, y1, x2, y2, r, g, b );			
		break;	

	case DRW_MUL:	
		ret = PRM_DrawFLine_T_Mul( dest, x1, y1, x2, y2, r, g, b );			
		break;	
									...
									RECT *clip, DWORD wparam )
{
	...
	if( !ClipRectLineRGB( &x1, &y1, &r1, &g1, &b1, &x2, &y2, &r2, &g2, &b2, dest->sx, dest->sy, clip ) ) 
		return 1;
	if( param==DRW_AMI2 || param== DRW_BLD2 )
	{
		if( blnd==BLD_0P   )
	...
	default:
	case DRW_NML:	
		ret = PRM_DrawGLine_T_Std( dest, x1, y1, r1, g1, b1, x2, y2, r2, g2, b2 );			
		break;	

	case DRW_BLD2:	
		ret = PRM_DrawGLine_T_Bld( dest, x1, y1, r1, g1, b1, x2, y2, r2, g2, b2, blnd );	
		break;

	case DRW_ADD:	
		ret = PRM_DrawGLine_T_Add( dest, x1, y1, r1, g1, b1, x2, y2, r2, g2, b2 );			
		break;

	case DRW_SUB:	
		ret = PRM_DrawGLine_T_Sub( dest, x1, y1, r1, g1, b1, x2, y2, r2, g2, b2 );			
		break;	

	case DRW_MUL:	
		ret = PRM_DrawGLine_T_Mul( dest, x1, y1, r1, g1, b1, x2, y2, r2, g2, b2 );			
		break;	
