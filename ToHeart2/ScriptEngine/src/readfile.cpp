...
int my_strcmpi(BYTE *str1,BYTE *str2)
{
	...
	min = len1 = lstrlen((LPCTSTR)str1);
	len2 = lstrlen((LPCTSTR)str2);
	if(len2 < min)
		min = len2;
	for(i=0;i<min;i++)
	{
...
int ClReadFile::OpenPackFile(LPCTSTR packName,int packNum)
{
	int			i,j;
	LACHEAD		head;
	HANDLE		handle;
	...
	wsprintf(filename,_T("%s.PAK"),packName);
...
int ClReadFile::ReadPackFile(int arcFileNum, LPCTSTR decFile, char **fileBuf)
{
	int		i;
#ifdef _WINDOWS	
	HANDLE	hFile;
	...
	int		out_size;
	TCHAR	filename[MAX_PATH];
	wsprintf(filename, _T("%s\\%s"), ArcFile[arcFileNum].dirName, decFile);
...
int ClReadFile::StreamOpenFile(int arcFileNum, LPCTSTR decFile,int &size)
{
	int		i;
...
ClResult ClReadFile::ExtractFile(int arcFileNum, LPCTSTR decFile, LPCTSTR newFile)
{
	char	*fileBuf;
	DWORD	Size,wSize;
...
TCHAR *ClReadFile::GetFileName(int arcFileNum,int num)
{
	if(num >= ArcFile[arcFileNum].fileCount) 
		return NULL;	
...
{
	int		i;
	ArcFile.resize(15); //ArcFile.reserve(15); //TODO:???
	for(i=0;i<15;i++)
	{
...
int ClReadFile::SearchFile(int arcFileNum, LPCTSTR decFile,BOOL errCheck)
{
	int		i,j;
	int		min,max;
...
#ifdef _DEBUG
		if(errCheck)
			MessageBox(NULL,decFile,_T("ファイルオープンエラー"),MB_OK|MB_ICONERROR);
#endif _DEBUG
	}
	return i;
} 

int ClReadFile::SequentialSearchFile(int arcFileNum, LPCTSTR decFile)
{	
	int i;
