...
BOOL CALLBACK ColorWarningDialog( HWND hwnd, UINT wmes, UINT wparam, LONG lparam )
{
	...
	case WM_COMMAND:
		...
		{
		case IDOK:
			if(hColorWarning[0]==hwnd)	
				ReCheckFlag[0] = SendDlgItemMessage(hwnd,IDC_CHECK1,BM_GETCHECK,0,0);
			else	
				ReCheckFlag[1] = SendDlgItemMessage(hwnd,IDC_CHECK1,BM_GETCHECK,0,0);
			EndDialog( hwnd, TRUE );
			break;
...
void SetFullScreenMenu( int menu )
{
	...
	if(Debug.flag)	
		SetWindowPos( MainWindow.hwnd, NULL, -mx, -my, 0, 0, SWP_NOSIZE|SWP_NOZORDER );
	else	
		SetWindowPos( MainWindow.hwnd, HWND_TOPMOST, -mx, -my, 0, 0, SWP_NOSIZE );
...
int	WIN_SetScreenMode( int w, int h, int rfrate )
{
	...
	if( w<0 || h<0 )
	{
		...
		if( ReadRegistryAmount( "pos_x", &px ) ) 
			px=0;
		if( ReadRegistryAmount( "pos_y", &py ) ) 
			py=0;
...
void WIN_InitMenu( void )
{
	...
	if(Avg.se_buf)	
		CheckMenuItem( hmenu, ID_SE_BUF,  MF_CHECKED );
	else			
		CheckMenuItem( hmenu, ID_SE_BUF,  MF_UNCHECKED );
	...
	{
	case PCM_44_16:	
		CheckMenuItem( hmenu, ID_44100_16, MF_CHECKED );	
		break;

	case PCM_22_16:
		CheckMenuItem( hmenu, ID_22050_16, MF_CHECKED );
		break;

	case PCM_11_16:	
		CheckMenuItem( hmenu, ID_11025_16, MF_CHECKED );
		break;

	case PCM_44_08:	
		CheckMenuItem( hmenu, ID_44100_8,  MF_CHECKED );	
		break;

	case PCM_22_08:	
		CheckMenuItem( hmenu, ID_22050_8,  MF_CHECKED );	
		break;

	case PCM_11_08:	
		CheckMenuItem( hmenu, ID_11025_8,  MF_CHECKED );
		break;
	}
	if( Avg.bgm )	
		CheckMenuItem( hmenu, ID_BGM_CHECK,   MF_CHECKED );
	if( Avg.se )
		CheckMenuItem( hmenu, ID_SE_CHECK,    MF_CHECKED );
	if( Avg.tx_pdwon )	
		CheckMenuItem( hmenu, ID_TEXT_PDOWN,  MF_CHECKED );
	if( Avg.tx_lbtrg )
		CheckMenuItem( hmenu, ID_TEXT_LBTRG,  MF_CHECKED );
	if(!Avg.mov1)
		EnableMenuItem( hmenu, ID_MOV_LOW,  MF_GRAYED );
	if(!Avg.mov2)
		EnableMenuItem( hmenu, ID_MOV_HIGH, MF_GRAYED );
	if(!Avg.mov3)
		EnableMenuItem( hmenu, ID_MOV_MAX,  MF_GRAYED );
...
static void MenuControl( HWND hwnd, UINT wparam )
{
	...
	case ID_SE_BUF:
		...
		if(Avg.se_buf)
		{
			DebugBox( MainWindow.hwnd, 
				"サウンドのバッファを多く確保することで、"
				"一部環境でのノイズ問題を解決します。\n"
				"ただし、BGMのレスポンスが悪くなるため、"
				"正常に音が聞ける環境でこの設定をすることにメリットはありません。\n"
				"問題のある方のみ設定してください。\n"
				"また、この設定は、ゲームをいったん終了し、"
				"再起動した後に有効になります。"  );
		}
		if(Avg.se_buf)	
			CheckMenuItem( hmenu, ID_SE_BUF,  MF_CHECKED );
	...
	case ID_TEXT_PDOWN:
		...
		if( Avg.tx_pdwon )	
			...
			CheckMenuItem( hmenu, ID_TEXT_PDOWN,  MF_UNCHECKED );
		break;
	...
	case ID_TEXT_LBTRG:
		...
		if( Avg.tx_lbtrg )	
			...
			CheckMenuItem( hmenu, ID_TEXT_LBTRG,  MF_UNCHECKED );
		break;
	...
	case ID_VOLUME:
		...
		if(0){
			...
			if( !hVolWin ){
				...
				{
					MessageBox2( NULL, MB_OK|MB_SETFOREGROUND, 
						"ボリュームコントロールが見つかりません", 
	...
	case ID_END:			
		...
		{
			if( MessageBox2( MainWindow.hwnd, MB_YESNO|MB_SETFOREGROUND|MB_SYSTEMMODAL, 
				"終了", 
				"%s を、終了してもよろしいですか？", TITLE_NAME )==IDYES )
			{
				MAIN_SetEnd();
				NotInitDirectDraw = 1;
			}
...
BOOL WIN_CreateVram( int direct_draw, int draw_mode )
{
	...
	if(draw_mode==-1 )
	{
		if(MainWindow.disp_mode==8)	
			...
			draw_mode = MainWindow.disp_mode;
	}
	else if(draw_mode==-2)
	{
	...
	if( (MainWindow.draw_mode == 8) && (MainWindow.draw_mode != MainWindow.draw_mode2) )
	{
		...
		{
		default:	
			DebugBox( NULL, "画面モードが異常です" );		
			break;

		case 16:	
			BMP_CreateBMP_H( &MainWindow.vram_high, w, h, 0 );
			break;

		case 24:	
			BMP_CreateBMP_F( &MainWindow.vram_full, w, h );	
			break;

		case 32:	
			BMP_CreateBMP_T( &MainWindow.vram_true, w, h );
			break;
		}
	}
	else
	{
		...
		case 16:
			BMP_GetImageVram16( MainWindow.hwnd, &MainWindow.hbmp, &MainWindow.vram_high, w, h );	
			break;

		case 24:
			BMP_GetImageVram24( MainWindow.hwnd, &MainWindow.MemDC, &MainWindow.hbmp, &MainWindow.hbmp_old, &MainWindow.vram_full, w, h );
			break;

		case 32:	
			BMP_GetImageVram32( MainWindow.hwnd, &MainWindow.MemDC, &MainWindow.hbmp, &MainWindow.hbmp_old, &MainWindow.vram_true, w, h );	
			break;
		}
	}
...
void WIN_Init( void )
{
	...
	{
	case PCM_44_16:
		SendMessage( MainWindow.hwnd, WM_COMMAND, ID_44100_16, 0 );	
		break;	

	case PCM_22_16:
		SendMessage( MainWindow.hwnd, WM_COMMAND, ID_22050_16, 0 );	
		break;	

	case PCM_11_16:
		SendMessage( MainWindow.hwnd, WM_COMMAND, ID_11025_16, 0 );
		break;	

	case PCM_44_08:
		SendMessage( MainWindow.hwnd, WM_COMMAND, ID_44100_8, 0 );	
		break;	

	case PCM_22_08:	
		SendMessage( MainWindow.hwnd, WM_COMMAND, ID_22050_8, 0 );
		break;	

	case PCM_11_08:
		SendMessage( MainWindow.hwnd, WM_COMMAND, ID_11025_8, 0 );
		break;	
	}
	AVG_InitSound();
}
...
int PASCAL WinMain( HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpszCmdParam,int nCmdShow )
{
	...
	if( !(GetAsyncKeyState(VK_NONCONVERT)&0x8000) )
	{
		while(1) 
		{
			...
			{					
				int flag = MessageBox2( NULL, MB_RETRYCANCEL, 
					"警告", 
					"DVD-ROMドライブに「%s」のディスクをセットして下さい", TITLE_NAME );
				if( flag != IDRETRY) 
					return 0;
			} 
			else
	...
	while( 1 ) 
	{
		...
		if( !MovPlaying && !MovPlaying2 )
		{
			...
			{
#if 0
				char		*_time_tbl[8] = { "挬","媥傒帪娫","拫媥傒","庼嬈拞","庼嬈廔椆屻","曻壽屻","栭","師偺擔傊" };
				char		*_youbi_tbl[7] = {"擔","寧","壩","悈","栘","嬥","搚"};
				int			_youbi=0;
				char		*_syukujitu_tbl[3] = { "","/廽擔","/弔媥傒"};
				int			_syukujitu=0;
#else
