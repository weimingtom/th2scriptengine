...
int PAC_PackFileMovie_Open( TCHAR *PackFname )
{
	int		i;
	int		size;
	for(i=0;i<32;i++)
	...
	if( mi->fh )
	{
		if(mi->cond==MOVE_COND_PLAY)
		{
			...
			{	
				...
				{
				case 0:	
					PAC_PackFileMovie_Start( move_no, MOVE_COND_PLAY, -1 );	
					break;
				
				case 1:	
					PAC_PackFileMovie_Start( move_no, MOVE_COND_END,  -1 );	
					break;
...
int  LZS_DecodeMemoryBmpTbl_WORD( WORD *dst, char *src, int file_size, int sabun, int bai, int rr, int gg, int bb )
{
	...
	src2 = src + sizeof(DWORD)*256;
	if(rr!=128 || gg!=128 || bb!=128)	
		BMP_ChangePalBright_TH( tbl, (RGB32*)src, rr, gg, bb );
	else								
		BMP_ChangePal_TH( tbl, (RGB32*)src );
	src_pos += sizeof(DWORD)*256;
	if(!bai)
	{
		if(!sabun)
